...
type
  ...
  TOutMgrOpCode = (omoUnk, omoReaddr, omoKill, omo_Immed, omo_Crash, omo_Direct,
    omo_Normal, omo_Hold, omoUnlink, omoPurge);
  ...
  TMailerForm = class(TForm)
    ...
    procedure PollsListViewChange(Sender: TObject; Item: TListItem; Change:
      TItemChange);
    ...
    procedure OutMgrOutlineDrawItem(Control: TWinControl; Index: Integer; R:
    ...
    procedure OutMgrHeaderSectionClick(HeaderControl: THeaderControl; Section:
      THeaderSection);
    procedure OutMgrHeaderSectionResize(HeaderControl: THeaderControl; Section:
      THeaderSection);
    ...
    procedure OutMgrOutlineKeyDown(Sender: TObject; var Key: Word; Shift:
      TShiftState);
    ...
    procedure PollsListViewKeyDown(Sender: TObject; var Key: Word; Shift:
      TShiftState);
  ...
  private
    ...
    wcb: set of Twcb;
    TopPagePanels: array[wcb_TopPage0..wcb_TopPage5] of TPanel;
    BtmPagePanels: array[wcb_BtmPage0..wcb_BtmPage5] of TPanel;
    ...
    procedure UpdateProt(const D: TDisplayData; const DS: TDisplayStringData; T,
      R: TBatch; AOutUsed: DWORD; var Btns: Boolean);
    ...
    procedure WMGetMinMaxInfo(var AMsg: TWmGetMinMaxInfo); message
      WM_GETMINMAXINFO;
    ...
    function GetOutFileColl(FileMask: PString; NodeAdrr: PFidoAddress;
      OutStatus: POutStatus; var AInfo: TOutMgrGroupInfo): TOutFileColl;
    procedure FillOutMgrSubMenu(AMenu: TMenuItem; C: TOutFileColl; const AInfo:
      TOutMgrGroupInfo);
    function GetOutCollByTag(ATag: Integer; Item: TOutItem; var Caption: string;
      var AInfo: TOutMgrGroupInfo): TOutFileColl;
    procedure PerformOutboundOperations(FileLists: TColl; DestAddr:
      PFidoAddress; OpCode: TOutMgrOpCode);
...
begin
  try
    ...
    MailerForms.Enter;
    try
      for i := 0 to CollMax(MailerForms) do
      begin
    ...
    SetProcessWorkingSetSize(GetCurrentProcess, DWORD(-1), DWORD(-1));
      ...
      ProcessTrap(E.Message, 'UpdateViewAll');
  end;
end;
...
begin
  ...
  for i := 0 to IniFile.NetmailAddrTo.Count - 1 do
  begin
    g.Add(Pad(IniFile.NetmailAddrTo[i], n + 2) + '<=  ' + ExtractWord(1,
      IniFile.NetmailAddrFrom[i], [' ']));
    t := IniFile.NetmailAddrFrom[i];
    for m := 2 to WordCount(t, [' ']) do
    begin
  ...
  begin
    r := Cfg.Passwords[i];
    for m := 0 to r.AddrList.Count - 1 do
    begin
      a := r.AddrList[m];
      if MatchMaskAddress(a, '*:*/*.0@*') then
      begin
...
begin
  ...
  begin
    n := O[i];
    g.Add(Pad(Addr2Str(n.Address), x + 3) + n.SizeString(y) + '   ' +
      n.AgeString);
  end;
  g.SaveToFile(S^);
  g.Free;
...
begin
  WinExec(PChar(string(IniFile.ExtApp.Table[TMenuItem(Sender).MenuIndex + 1,
    1])), sw_shownormal);
end;
...
begin
  if FileAge(BWZColl.FName) <> BWZColl.Age then
    PostMSG(WM_UPDBWZ);
end;
...
var
  ...
  function CreateMenuItem(const menucaption: string; menuenabled: boolean;
    shortcutidx: integer): TMenuItem;
  begin
    result := TMenuItem.Create(self);
    result.Caption := menucaption;
    result.OnClick := MyMenuClick;
    result.Enabled := menuenabled;
    if (shortcutidx > 0) and (shortcutidx < 10) then
      result.ShortCut := 16432 + shortcutidx;
  end;
...
begin
  ...
  begin
    tExternal.Insert(i - 1, CreateMenuItem(IniFile.ExtApp.Table[i, 2], true,
      i));
  end;
end;
...
begin
  MailerForms.Enter;
  try
    if MainWinHandle <> 0 then
    begin
      try
        KillTimer(MainWinHandle, 1);
        DeallocateHWnd(MainWinHandle);
        MainWinHandle := 0;
      except
        on E: Exception do
          ProcessTrap(IntToStr(MainWinHandle) + ' ' + E.Message, ClassName);
      end;
    end;
    inherited Destroy;
  finally
...
procedure NewIPLine(APort: Pointer; AIpPort: Integer; AProt: TProtCore; AType:
  DWORD);
var
  ILD: TNewIpLineData;
  cw,
...
var
  ...
  m: TMailerThread;
begin
  if FidoPolls.IndexOf(cr.p) > -1 then
  begin
    if cr.Error then
    begin
      z := TFidoPoll(cr.p).IPAddr;
      if cr.Prot = ptSMTP then
      begin
        ...
        begin
          ...
          begin
            ...
            if MatchMaskAddressListSingle(TFidoPoll(cr.p).Node.Addr,
              ExtractWord(5, s, ['|'])) then
            begin
              z := ExtractWord(1, s, ['|']);
              break;
            end;
...
procedure UpdThrLog;
begin
  if TimerInstalled(ThrLogTimer) and not TimerExpired(ThrLogTimer) then
    Exit;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        DeleteFile(FClose);
        MailerThreads.Enter;
        for j := 0 to CollMax(MailerThreads) do
        begin
          t := MailerThreads[j];
          ...
          begin
            ...
            begin
              (Application.MainForm as TMailerForm).TrayIcon.Hint := LR.Name +
                ' closed';
            end;
            Sleep(2000);
          end;
  ...
  var
    ...
    begin
      ...
      else if copy(s, 1, 8) = '%RESTART' then
      begin
        ...
        if (length(s) > 1) or (strtointdef(copy(s, 1, pos('%', s) - 1), 0) <
          5000) then
          s := '5000%';
        StoreConfig(0);
        ShellExecute(0, nil, PChar(ParamStr(0)),
          PChar('delay' + copy(s, 1, pos('%', s) - 1)),
  ...
  begin
    case xBase.DirExists(IniFile.FlagsDir) of
      -1: FidoPolls.Log.LogSelf(Format('Please remove file ' + IniFile.FlagsDir +
          ' from the %s directory', [HomeDir]));
    ...
    begin
      DeleteFile(s);
      FidoPolls.Log.LogSelf(Format('Detected %s - forced nodelist compilation',
        [s]));
    ...
    begin
      DeleteFile(s);
      FidoPolls.Log.LogSelf(Format('Detected %s - forced password list import',
        [s]));
        ...
        case c of
          '?':
            begin
              if _FileExists(s) then
              begin
                FidoPolls.Log.LogSelf(Format('Detected %s - forced executing %s',
                  [s, e]));
        ...
        case c of
          ...
          '&':
            begin
              if not _FileExists(s) then
              begin
                ShellExecute(Application.Handle, nil, PChar(format('/? > %s',
                  [s])),
...
begin
  if not forced then
  begin
    if TimerInstalled(FlFlgTimer) and not TimerExpired(FlFlgTimer) then
      Exit;
  end;
...
procedure DeleteFlag(const s: string);
begin
  DelFile('DeleteFlag', MakeNormName(IniFile.FlagsDir, s));
end;
...
begin
  ...
  if (Msg.Msg in [WM_QUERYENDSESSION, WM_ENDSESSION]) and
    (IniFile.IgnoreEndSession) then
  begin
    MSG.Result := 1;
    exit;
  end;
      ...
      PBT_APMQUERYSUSPEND:
        begin
          ...
          begin
            ...
            if MlrT.DialupLine then
            begin
              MlrT.Enter;
              if (MlrT.SD <> nil) and (MlrT.SD.Prot <> nil) then
              begin
  ...
  if RASConnect and (RASThread <> nil) then
  ...
  try
    ...
    begin
      ...
      else if Msg.WParam = 8 then
      begin
        Msg.Result := (GetTickCount - StartTime) div 1000;
        exit;
      end
      else if Msg.WParamLo = 9 then
      begin
        ...
        cd.cbData := Length(s);
        cd.lpData := @s[1];
        if SendMessage(APIHandle, WM_COPYDATA, MainWinHandle, integer(@cd)) <> 0 then
          Msg.Result := 1
        else
          Msg.Result := 0;
        exit;
      ...
      if Line >= DWORD(Cfg.Lines.Count) then
      begin
        Msg.Result := 0;
        exit;
      end;
      ...
      begin
        MlrT := MailerThreads[i];
        if DWORD(l.Id) = MlrT.LineId then
        begin
          break;
        end
        else
      ...
      begin
        case Msg.WParam and $FFFF of
          1:
            begin
              if MlrT.State in [__FirstMisc..__LastMisc] then
              begin
                if MlrT.SD <> nil then
                begin
      ...
      else if Msg.WParam and $FFFF = 3 then
      begin
        MlrT := OpenMailer(l.Id, MainWinHandle);
        if MlrT <> nil then
        begin
          MailerForms.Enter;
          if MailerForms.Count > 0 then
          begin
    ...
    case Msg.Msg of
      WM_UPDOUTMGR,
        ...
        WM_UPDBWZ:
        repeat
          if not PeekMessage(DummyMsg, MainWinHandle, Msg.Msg, Msg.Msg,
            PM_REMOVE) then
          begin
            Break;
          end;
    ...
    case Msg.Msg of
      ...
      WM_TRAYICON: if (Application.MainForm as TMailerForm).TrayIcon <> nil then
          (Application.MainForm as TMailerForm).TrayIcon.SeparateIcon :=
            IniFile.AlwaysInTray;
      ...
      WM_CONNECT:
        begin
          if ApplicationDowned and Inifile.TrayLamps then
          begin
      ...
      WM_RESOLVE..WM_RESOLVE + WM__NUMRESOLVE - 1: HostResolveComplete(Msg.Msg -
          WM_RESOLVE, Msg.lParam);
        ...
        begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if TMailerThread(MailerThreads[LinesCnt]).LineId =
                        RASMailLin then
                        exit;
                    end;
                    repeat
          ...
          begin
            case msg.LParam of
              ...
              if (RASThread <> nil) and (RASInvoked or DaemonStarted) then
                RASThread.Connect(True);
            end;
          end;
        end;
      WM_RASEVENT:
        begin
          DeleteFlag('RASEVENT.OK');
          RASConnect := True;
      ...
      begin
        Msg.Result := DefWindowProc(MainWinHandle, Msg.Msg, Msg.wParam,
          Msg.lParam);
        Exit;
      end;
    end;
    Msg.Result := 0;
      ...
      ProcessTrap(E.Message + '(Message: ' + IntToStr(Msg.Msg) + ')',
        'MainVCL');
    {.$142C}
  end;
end;
...
begin
  ...
  begin
    ...
    if not ONode.Expanded then
      Continue;
    New(pa);
    pa^ := n.Address;
    if OutMgrExpanded = nil then
      OutMgrExpanded := TFidoAddrColl.Create;
...
begin
  ...
  if IniFile.Stealth or IniFile.HideInTray then
  begin
    WindowState := wsMinimized;
    Application.ShowMainForm := False;
    if IniFile.HideInTray then
    begin
      FormActivate(Self);
...
begin
  ...
  if not Windows.CopyFile(PChar(Src), PChar(Dst), False) then
    DisplayError(FormatErrorMsg(Format('%s -> %s', [Src, Dst]), GetLastError),
      Handle);
end;
...
begin
  ...
  begin
    if WinDlgCap(FormatLng(rsMMNewCfgItems, [ProductNameFull, ProductVersion,
      Cfg.UpgStrings.LongString]), MB_YESNO or MB_ICONQUESTION, MailerForm.Handle,
        LngStr(rsMMCfgUpd)) = idYes then
      BackupConfig(MailerForm.Handle);
...
begin
  ...
  begin
    ...
    if not m.DialupLine then
      Continue;
    if ActiveLine = m then
      ti := c;
    DoAdd(m.Name);
  end;
  if ti = -1 then
    ti := c;
  if ActiveLine = PanelOwnerPolls then
    ti := c;
  DoAdd(LngStr(rsMMTabPolls));
  if ActiveLine = PanelOwnerOutMgr then
    ti := c;
  DoAdd(LngStr(rsMMTabOutbound));
  if ActiveLine = PanelOwnerSystem then
    ti := c;
  DoAdd(LngStr(rsMMTabSystem));
  if DaemonStarted then
  begin
    if ActiveLine = PanelOwnerDaemon then
      ti := c;
...
begin
  ...
  begin
    Lbl.Caption := s;
    if Lbl.ShowHint then
      Lbl.Hint := s;
  end
  else if L is TMenuItem then
    Mnu.Caption := s
  else
...
const
  bsMsg: array[TBatchState] of Integer = (rsMMbsInit, rsMMbsDummy, rsMMbsEnd,
    rsMMbsIdle, rsMMbsWait);
...
var
  ...
  begin
    if (IniFile.DontLogTariff) or (not (ActiveLine is TMailerThread)) then
      exit;
    ...
    begin
      ...
      if not ActiveLine.DialupLine then
        ...
        else if not (ActiveLine is TMailerThread) then
          exit;
      if ActiveLine.SD = nil then
        exit;
      if ActiveLine.SD.ActivePoll = nil then
      ...
      Period.StartTime := uDelphiTime(ActiveLine.SD.ConnectStart +
        ZoneTimeDiff);
...
begin
  ...
  if (ActiveLine.SD.Prot <> nil) and
    (CollCount(ActiveLine.SD.Prot.RemFiles) > 1) then
  begin
    if gLst.Tag = 0 then
    begin
      gLst.ColWidths[1] := gLst.Width - 4 - gLst.ColWidths[0] - gLst.ColWidths[2]
        - GetsystemMetrics(SM_CXVSCROLL);
    end;

    DoFill(ActiveLine.SD.prot.RemFiles);

    if gTabCnt.Tag <> 7 then
    begin
      gTabCnt.Tag := 7;
      gNfo.DefaultRowHeight := (gNfo.Height - GetSystemMetrics(SM_CYHSCROLL) - 9)
        div 8;
  ...
  begin
    if gTabCnt.TabIndex = 1 then
    begin
      gTabCnt.TabIndex := 0;
      gTabCntChange(gTabCnt);
    end;
    if gTabCnt.Tabs.Count > 0 then
    begin
      gTabCnt.Tabs.Clear;
    end;
    if gTabCnt.Tag <> 9 then
    begin
      gTabCnt.Tag := 9;
      gNfo.DefaultRowHeight := (gNfo.Height - GetSystemMetrics(SM_CYHSCROLL) - 9)
        div 8;
  ...
  with DS do
    DoFill([ConnectString, rmtStationName, rmtAddressList, rmtSysopName,
      rmtLocation, rmtPhone, rmtFlags, rmtSoft, ActiveLine.SD.rmtTime]); //visual

  lSessionTime.Caption := FormatDateTime('hh:mm:ss',
    EncodeTime(((activeline.timeonline div 1000) div 60) div 60,
    ((activeline.timeonline div 1000) div 60) mod 60, (activeline.timeonline div
    1000) mod 60, activeline.timeonline mod 1000));
  SetSessionTime(True);
  ...
  begin
    ...
    begin
      ...
      if fpos < T.D.FOfs then
        ...
        a := fpos - T.D.FOfs;
      SetLabel(lSndSize, wcsSndSize, Format('%s / %s', [Int2Str(MinD(fpos,
          T.D.FSize)), Int2Str(T.D.FSize)]));
      SetSndSize(True);
      ...
      begin
        ...
        begin
          ...
          SetLabel(lFileSndTime, wcslFileSndTime, FormatDateTime('hh:mm:ss',
            Seconds / 86400));
          SetSndTime(True);
        end;
      end;
  ...
  o := False;
  if i + lSndFile.Left > lSndCPS.Left then
  begin
    o := True;
    while lSndFile.Canvas.TextWidth(s + '..') + lSndFile.Left > lSndCPS.Left do
    begin
  ...
  begin
    ...
    begin
      ...
      SetLabel(lRcvSize, wcsRcvSize, Format('%s / %s', [Int2Str(MinD(R.D.FPos +
          R.D.Part, R.D.FSize)), Int2Str(R.D.FSize)]));
      SetRcvSize(True);
      if (ela < IniFile.CPS_MinSecs) or (dword(a) < IniFile.CPS_MinBytes) then
      begin
        SetRcvCPS(False);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            SetLabel(lFileRcvTime, wcslFileRcvTime, FormatDateTime('hh:mm:ss',
              Seconds / 86400));
            SetRcvTime(True);
          end;
        end;
  ...
  o := False;
  if i + lRcvFile.Left > lRcvCPS.Left then
  begin
    o := True;
    while lRcvFile.Canvas.TextWidth(s + '..') + lRcvFile.Left > lRcvCPS.Left do
    begin
      s := copy(s, 1, Length(s) - 1);
    end;
    s := s + '..';
  ...
  begin
    ...
    SetLabel(lTotalSndTime, wcslTotalSndTime, FormatDateTime('hh:mm:ss', Seconds
      / 86400));
  end;
  {   end;}
  a := D.rmtForUs;
  ...
  begin
    ...
    SetLabel(lTotalRcvTime, wcslTotalRcvTime, FormatDateTime('hh:mm:ss', Seconds
      / 86400));
  end;
  {   end;}
  CalcSessionCost;
...
procedure TMailerForm.UpdateDial(const D: TDisplayData; const DS:
  TDisplayStringData);
var
  e: DWORD;
  s: string;
begin
  ...
  else if pos(';', DS.StatusParam) = 0 then
    s := FormatLng(D.StatusMsg, [DS.StatusParam])
  else
    s := FormatLng(D.StatusMsg, [wizard.ExtractWord(1, DS.StatusParam, [';']),
      wizard.ExtractWord(2, DS.StatusParam, [';'])]);
...
var
  xy: array[0..5] of record x, y, n: integer;
    b: boolean
  end =
  ((x: 0; y: 0; n: 0; b: False), (x: 5; y: 0; n: 1; b: False), (x: 10; y: 0; n:
    1; b: False),
    (x: 0; y: 7; n: 1; b: False), (x: 5; y: 7; n: 0; b: False), (x: 10; y: 7; n:
    0; b: False));
...
begin
  if (ActiveLine = PanelOwnerPolls) or
    ...
    (MailerThreads.IndexOf(ActiveLine) = -1) then
  begin
    if (Application.MainForm <> nil) and ((Application.MainForm as
      TMailerForm).TrayIcon <> nil) then
    begin
      (Application.MainForm as TMailerForm).TrayIcon.Icon := Application.Icon;
    end;
    if Inifile.TrayLamps and ApplicationDowned then
    begin
      OK := True;
      for j := 0 to CollMax(MailerThreads) do
      begin
  ...
  if ilTray = nil then
    exit;
  if Inifile.TrayLamps and (ApplicationDowned or Inifile.AlwaysInTray) and (TTim
    + 50 < GetTickCount) then
  begin
    ...
    for j := 0 to 5 do
    begin
      ilTray.Draw(bm.Canvas, xy[j].x, xy[j].y, integer(xy[j].b) * xy[j].n + 2 *
        (1 - integer(xy[j].b)), true);
    end;
...
begin
  if P1.Birth = P2.Birth then
    Result := 0
  else if P1.Birth < P2.Birth then
    Result := 1
  else
    Result := -1;
end;
    ...
    2:
      begin
        if N1.Nfo.Size = N2.Nfo.Size then
          Cmp0
        else if N1.Nfo.Size < N2.Nfo.Size then
          C := 1
        else
          C := -1;
      end;
    3:
      ...
      begin
        if N1.Nfo.Attr = N2.Nfo.Attr then
          Cmp0
        else if N1.Nfo.Attr < N2.Nfo.Attr then
          C := 1
        else
          C := -1;
      end;
    ...
    5:
      begin
        if N1.Nfo.Time = N2.Nfo.Time then
          Cmp0
        else if N1.Nfo.Time < N2.Nfo.Time then
          C := 1
        else
          C := -1;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if not (S.wMonth - 1 in ol.CronRec.p^[j].Months) then
            continue;
          if not (S.wDay - 1 in ol.CronRec.p^[j].Days) then
            continue;
          if not (S.wDayOfWeek in ol.CronRec.p^[j].Dows) then
            continue;
...
begin
  ...
  if cc >= 0 then
    OutMgrNodes.Sort(DoOutMgrNodeSort);
  for I := 0 to cc do
  begin
    n := OutMgrNodes[I];
    if n.Files <> nil then
      n.Files.Sort(DoOutMgrNodeSort);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        Attr := 0;
        if i = cc then
          Attr := Attr or olfLastLevel;
        if k = ccc then
          Attr := Attr or olfLastItem;
      end;
  ...
  begin
    ...
    if not nn.HasItems then
      Continue;
    if (OutMgrExpanded <> nil) and (OutMgrExpanded.Search(@n.Address, cc)) then
      nn.Expanded := True;
  end;
  if sitem > 0 then
    OutMgrOutline.SelectedItem := sitem;
  SendMessage(OutMgrOutline.Handle, WM_VSCROLL, SB_THUMBPOSITION or sp shl 16,
    0);
...
procedure TMailerForm.UpdateView(fromcc: boolean);
  ...
  begin
    ...
    begin
      if (ActiveLine.SD.Prot.Chat <> nil) then
      begin
        try
          if ChatMemo1.Text <> ActiveLine.SD.Prot.Chat.Memo1Text.Text then
          begin
            ChatMemo1.Text := ActiveLine.SD.Prot.Chat.Memo1Text.Text;
            ChatMemo1.Perform(em_linescroll, 0, ChatMemo1.Lines.Count);
          end;
          if ChatMemo2.Text <> ActiveLine.SD.Prot.Chat.Memo2Text.Text then
          begin
            ChatMemo2.Text := ActiveLine.SD.Prot.Chat.Memo2Text.Text;
            ChatMemo2.Perform(em_linescroll, 0, ChatMemo2.Lines.Count);
          end;
          ChatPan.Visible := (ActiveLine.SD.Prot.Chat <> nil) and
            (ActiveLine.SD.Prot.Chat.Visible);
          if ChatPan.Visible then
          begin
        ...
        ChatPan.Visible := false;
      SetEnabledO(mlChat, wcb_mlChat, ActiveLine.SD.Prot.CanChat and not
        ActiveLine.SD.Prot.ChatOpened);
      SetEnabledO(bChat, wcb_bChat, ActiveLine.SD.Prot.CanChat and not
        ActiveLine.SD.Prot.ChatOpened);
    end
    else
    begin
  ...
  begin
    ...
    begin
      RasThread.CheckConnection;
      UpDateRas(uFormatDateTime('hh:mm:ss', RasThread.Lasts div 1000),
        RasThread.ResultString);
      if RASThread.EntryName = '' then
      begin
      ...
      begin
        ConnectButton.Enabled := RASThread.ResultString = 'Idle';
        CancelButton.Enabled := not ConnectButton.Enabled;
      end;
    end
    else
    begin
  ...
  const
    PT: array[TPollType] of Integer = (-1, rsMMptAuto, rsMMptNetm, rsMMptCron,
      rsMMptTest, rsMMptManual, rsMMptImm, rsMMptBack, rsMMptNmIm);
  var
    C: TColl;
    S: TStringColl;
    I,
  ...
  begin
    ...
    begin
      ...
      else if (P.Owner = nil) or (P.Owner = PollOwnerExtApp) then
        N := rsMMpsIdle
      else if P.Owner = PollOwnerDaemon then
        N := rsMMpsConnect
      else
        case
          P.Owner.State of
  ...
  begin
    ...
    begin
      SetEnabledO(mlSendMdmCmds, wcb_mlSendMdmCmds, B and
        (AllowedMdmCmdState(ActiveLine.State) { and (ActiveLine.CP <> nil)}));
      mnuTerminal.Enabled := B and AllowedMdmCmdState(ActiveLine.State) and
        ((@F_OpenTerm <> nil) or (@F_OpenUniTerm <> nil));
    end;
    if B then
    begin
      if (ActiveLine = PanelOwnerPolls) or
        (ActiveLine = PanelOwnerOutMgr) or
        (ActiveLine = PanelOwnerSystem) or
        (ActiveLine = PanelOwnerDaemon) then
        GlobalFail('%s', ['SetLineCommands']);
    ...
    begin
      ...
      if mpPause.Caption <> s then
      begin
        mpPause.Caption := s;
      end;
      if ppPause.Caption <> s then
      begin
        ppPause.Caption := s;
      end;
    end;
  ...
  begin
    if exitnow then
      exit;
    if gOutputGraph = nil then
      exit;
    if gInputGraph = nil then
      exit;
    if TCPIP_GrCS.DebugInfo = nil then
      exit;
    EnterCS(TCPIP_GrCS);
    try
  ...
  begin
    ...
    begin
      ...
      a.DropTarget := False;
      if a.SubItems[0] <> m.StatusMsg then
      begin
      ...
      begin
        ...
        begin
          ...
          r := FormatDateTime('hh:mm:ss', EncodeTime(((o div 1000) div 60) div
            60, ((o div 1000) div 60) mod 60, (o div 1000) mod 60, o mod 1000));
        end
        else
        begin
          r := '';
      ...
      if a.SubItems[1] <> r then
      begin
        a.SubItems[1] := r;
      end;
    end;
    MailerThreads.Leave;
    for i := stListView.Items.Count - 1 downto 0 do
    begin
      if stListView.Items[i].DropTarget then
      begin
    ...
    if t <> '' then
    begin
      stListView.ItemFocused := stListView.FindCaption(0, t, False, True,
        False);
      if stListView.ItemFocused <> nil then
      begin
...
begin
  ...
  begin
    try
      if IniFile.InSecure[2] <> ':' then
        ...
        dsk := ord(Upcase(IniFile.InSecure[1])) - $40;
      GroupBox2.Caption := ' badwazoo / free at inbound: ' +
        FloatToStr(Round(DiskFree(dsk) / 1048576)) + ' Mb';
    except on E: Exception do
        ProcessTrap(E.Message, 'Disk space check');
    end;
  end;
  try
    ...
    begin
      ...
      ProcessTrap(E.Message, 'Disk space check #2');
  end;
  {---}
  ClearMlr := True;
  B := False;
  ...
  if IniFile.ConfigReaded then
    if Trim(IniFile.ReadString('LogNames', 'uptime.log')) <> '' then
    begin
...
//ShowMessage(UpTimer);
      ...
      if UpTimerHandle = INVALID_HANDLE_VALUE then
      begin  
//        IniFile.WriteConfig;
//ShowMessage('UpTimerHandle = INVALID_HANDLE_VALUE');
//        RaiseLastWin32Error;
      end;
      SetFilePointer(UpTimerHandle, 0, nil, FILE_BEGIN);
      WriteFile(UpTimerHandle, UpTimer[1], Length(UpTimer), J, nil);
      SetEndOfFile(UpTimerHandle);
      CloseHandle(UpTimerHandle);
      Sleep(0);
    end;
  FormResize(nil);
end;
...
begin
  ...
  case Integer(ActiveLine) of
    ...
    begin
      ...
      DoClearTerms := False;
      s := ActiveLine.Name;
      if ActiveLine.PublicD.isZMH then
      begin
        s := s + ' (ZMH)';
      end;
      LogBox.Visible := true;
      if (ActiveLine.SD <> nil) and (ActiveLine.SD.Prot <> nil) and
  ...
  begin
    if not WMCLOSE then
      case IniFile.OnClose of
        1:
          begin
            if MessageBox(handle, PChar(lngstr(rsCloseQuestion)), 'Mailer',
    ...
    SetEvt(oShutdown);
    if not KillTimer(MainWinHandle, 1) then
      GlobalFail('KillTimer Error %d', [GetLastError]);
    if not KillTimer(Handle, 1) then
      GlobalFail('KillTimer Error %d', [GetLastError]);
    ...
    if not GetWindowPlacement(handle, @wp) then
      GlobalFail('WindowPlacement error %d', [GetLastError]);
    if WindowState = wsMaximized then
    begin
      x3.Maximized := 1;
      if wp.rcNormalPosition.Left > -(wp.rcNormalPosition.Right -
        wp.rcNormalPosition.Left) then //DV: 07.08.01
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Handle <> INVALID_HANDLE_VALUE then
          DisplayErrorLng(rsMMAlrAct, Handle);
        Break
      end;
    end;
    if j = -1 then
    begin
      ...
      begin
        ...
        begin
          if Handle <> INVALID_HANDLE_VALUE then
            DisplayErrorFmtLng(rsMMCantOpenTDev, [R.Name], Handle);
        end
        else
        begin
          if Handle <> INVALID_HANDLE_VALUE then
            DisplayErrorFmtLng(rsMMCantOpenPort, [ComName(R.d.Port)], Handle);
        end;
        Exit;
      end;
      GetTermBounds(cw, ch);
...
begin
  ...
  begin
    ...
    if pos('&', s) > 0 then
      Delete(s, pos('&', TMenuItem(Sender).Caption), 1);
    idx := _MT.IndexOf(s);
    if (idx > -1) then
    begin
...
begin
  case an.PrefixFlag of
    ...
    nfHold,
      ...
      begin
        Result :=
          YesNoConfirm(Format('Node %s (%s from %s) has %s status in the nodelist. Create poll anyway?', [Addr2Str(an.Addr), an.Sysop, an.Location, cNodePrefixFlag[an.PrefixFlag]]),
          Handle)
      end;
  else
    Result := True;
...
begin
  ...
  mbp.lpszText := PChar(
    ProductNameFull +
    #10 + StringOfChar(#$97, 28) + #10 +
    'Version: ' + CProductVersionForAbout +
    #10 + StringOfChar(#$97, 28) + #10#10 +
    #169' 2009-' + FormatDateTime('yyyy', Now) + ' by Vladimir A.Bakhvaloff. All rights reserved'#10 +
    'FidoNet: 2:5030/535, e-mail: bob@bakh.spb.ru'#10#10 +
    #169' 2003-2005 by Taurus. All rights reserved'#10 +
    'FidoNet: 2:461/700, 701, 702, e-mail: taurus@rinet.ru'#10#10 +
    #169' 2003-2004 by RadSoft. All rights reserved'#10#10 +
    #169' 2000-2003 by Denis Voituk. All rights reserved'#10 +
    'FidoNet: 2:5012/37, 2:5012/38, 2:550/5012, e-mail: deniska666@mail.ru'#10#10 +
    #169' 2001 OlegON software. All rights reserved'#10 +
    'FidoNet: 2:5020/1970, e-mail: support@olegon.com'#10#10 +
    #169' 2001-2002 by Sergey Shumakov. All rights reserved'#10 +
    'FidoNet: 2:5005/47'#10#10 +
    #169' 1996-2001 RITLABS S.R.L. All rights reserved'#10#10 +
    'Hydra transfer protocol'#10 +
    #169' 1991-1993 Joaquim H. Homrighausen. All rights reserved'#10 +
    #169' 1991-1993 Lentz Software Development. All rights reserved'#10#10 +
    'Contains cryptographic software written by Eric Young');

  mbp.lpszCaption := 'About Taurus';
  mbp.hInstance := hInstance;
  mbp.lpszIcon := MAKEINTRESOURCE(700);
  mbp.dwStyle := MB_USERICON or MB_OK;
  mbp.dwContextHelpId := 0;
  mbp.dwLanguageId := MAKELANGID(LANG_NEUTRAL, 0);
...
begin
  ...
  begin
    ...
    if not ParseAddress(s, a) then
      GlobalFail('TMailerForm.bDeletePollClick, failed to parse "%s"', [s]);
    EnterFidoPolls;
...
begin
  ...
  begin
    s := li.Caption;
    if not ParseAddress(s, a) then
      GlobalFail('TMailerForm.bResetPollClick, failed to parse "%s"', [s]);
    EnterFidoPolls;
    for i := 0 to FidoPolls.Count - 1 do
    begin
      p := FidoPolls[i];
...
procedure TMailerForm.LoadOutMgrBmp(i: Integer; const AName: string; AColor:
  TColor);
var
  B,
    C: TBitmap;
...
begin
  if not IniFile.ShowMenuIcons then
  begin
    MainMenu.Images := nil;
    TrayPopupMenu.Images := nil;
  end;
  lInfo1.Caption :=
  ...
  begin
    ...
    if ((IniFile.RadMF.Bounds[0] <> 32767) and
      ...
      (IniFile.RadMF.Bounds[0] < screen.Width) and (IniFile.RadMF.Bounds[1] +
      IniFile.RadMF.Bounds[3] > 0) then
      SetBounds(IniFile.RadMF.Bounds[0], IniFile.RadMF.Bounds[1],
        IniFile.RadMF.Bounds[2], IniFile.RadMF.Bounds[3]);
    if IniFile.RadMF.Maximized = 1 then
  ...
  begin
    ...
    STListView.Columns[0].Width := SavFile.ReadInteger('Sizes', 'stListView',
      250);
...
var
  h: THandle;
begin
  if (ActiveLine is TMailerThread) and (ActiveLine.DialupLine) then
  begin
    if @F_OpenUniTerm <> nil then
    begin
      if ActiveLine.TapiDevice then
      begin
...
begin
  ...
  begin
    ...
    if IniFile = nil then
      exit;
    if not IniFile.DynamicOutbound then
      exit;
    MailerThreads.Enter;
    try
      for i := 0 to CollMax(MailerThreads) do
      begin
        ...
        if t.SD = nil then
          continue;
        if not t.SD.WeHaveReported then
          continue;
        if t.SD.OutFiles = nil then
          continue;
        if not t.D.Initialized then
          continue;
        if CollCount(t.SD.rmtAddrs) = 0 then
          continue;
...
begin
  if YesNoConfirmLng(rsMMokCompileNdl, Handle) then
    VCompileNodelist(False);
end;
...
var
  ...
  begin
    case p.Node.PrefixFlag of
      nfPvt, nfHold, nfDown: s := Format(' (%s)',
          [cNodePrefixFlag[p.Node.PrefixFlag]]);
    end;
    ...
    begin
      SC.Add(FormatLng(rsMMStationHdrB, [p.Node.Station, Addr2Str(p.Node.Addr)])
        + s);
...
const
  PtpTyp: array[TPollType] of Integer = (0, rsMMptpiOutb, rsMMptpiNetm,
    rsMMptpiCron, rsMMptpiTest, rsMMptpiManual, rsMMptpiImm, rsMMptpiBack,
    rsMMptpiNmIm);
...
begin
  ...
  begin
    s := li.Caption;
    if not ParseAddress(s, a) then
      GlobalFail('TMailerForm.bTracePollClick, failed to parse "%s"', [s]);
    EnterFidoPolls;
    try
      for i := 0 to FidoPolls.Count - 1 do
      begin
        ...
        begin
          ...
          for ii := 0 to CollMax(MailerThreads) do
          begin
            m := MailerThreads[ii];
            if not m.DialupLine then
              Continue;
...
begin
  ...
  begin
    if MessageBox(handle, 'Do you really want to skip that file?', 'Mailer',
...
begin
  ...
  begin
    if MessageBox(handle, 'Do you really want to reject that file?', 'Mailer',
      mb_OKCANCEL or MB_ICONQUESTION) = idok then
...
procedure TMailerForm.OutMgrOutlineDrawItem(Control: TWinControl; Index:
  Integer; R: TRect; State: TOwnerDrawState);
var
  I: Integer;
  F: TOutItem;
  N: TOutlineNode;
  ...
  begin
    if R.Left < R.Right then
      DrawText(OutMgrBM.Canvas.Handle, PChar(AStr), Length(AStr), R,
        DT_END_ELLIPSIS or DT_NOCLIP or DT_NOPREFIX or AFlags);
  end;
...
begin
  ...
  F := N.Data;
  if OutMgrBM.Height < R.Bottom - R.Top then
    OutMgrBM.Height := R.Bottom - R.Top;
  if OutMgrBM.Width < R.Right - R.Left then
    OutMgrBM.Width := R.Right - R.Left;
  RR := R;
  ...
  begin
    ...
    FillRect(Rect(R.Left, R.Top, R.Left + (Integer(N.Level) + 1) * 16,
      R.Bottom));
    ClrRct(R);
    if N.Level = 1 then
    begin
    ...
    begin
      if (f.Nfo.Attr and olfLastItem = 0) then
        ...
        I := 1;
      if (f.Nfo.Attr and olfLastLevel = 0) then
        BitBlt(Handle, R.Left, R.Top, 16, 16, OutMgrBmps[ioLines].Canvas.Handle,
          144, 0, SRCCOPY);
      Inc(R.Left, 16);
    end;

    ClrRct(R);
    BitBlt(Handle, R.Left, R.Top, 16, 16, OutMgrBmps[ioLines].Canvas.Handle, I *
      16, 0, SRCCOPY);
    ...
    begin
      ...
      begin
        ...
        if osBusyEx in F.StatusSet then
          s := FormatLng(rsMMOutNCusy, [Addr2Str(F.Address)]);
        //          s := Format('%s [%s]',[s,s2]);
        s := Format('%s', [s]);
      end
      else
        ...
        case F.Status of
          ...
          osImmed, osCrash, osDirect, osNormal, osHold:
            begin
              ...
              else if IsArcMailExt(zzz) then
              begin
                I := 4;
                s := ExtractFileName(f.Name);
              end
              else if (UpperCase(zzz) = '.PKT') or (UpperCase(zzz) = '.P2K') then
              begin
                I := 2;
              end
              else
    ...
    Inc(R.Left, 16);
    ClrRct(R);
    BitBlt(Handle, R.Left, R.Top, 16, 16, OutMgrBmps[ioFolders].Canvas.Handle, I
      * 16, 0, SRCCOPY);
    Inc(R.Left, 16);
...
begin
  if (Application.MainForm = nil) or (MainPanel.Parent = nil) then
    exit;
  UpdateBoundsOutMgrBM;
...
procedure TMailerForm.OutMgrOutlineMouseDown(Sender: TObject; Button:
...
procedure TMailerForm.OutMgrHeaderSectionClick(HeaderControl: THeaderControl;
...
procedure TMailerForm.OutMgrHeaderSectionResize(HeaderControl: THeaderControl;
  Section: THeaderSection);
begin
  OutMgrRefillExpanded;
  UpdateViewOutMgr;
...
begin
  ...
  if (Win32Platform = VER_PLATFORM_WIN32_NT) then
  begin
    if aOutbound.CommonAVI <> aviFindFolder then
      aOutbound.CommonAVI := aviFindFolder;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        MailerForms.Enter;
        if MainTabControl.Tabs.Count > TNum then
        begin
          if MainTabControl.Tabs[TNum] = TNam then
          begin
...
begin
  if (Cfg.MasterKey <> 0) and (ReloginFailed) then
    Exit;
  SetupEncryptedLinks;
end;
...
procedure TMailerForm.CreateOutFileFlag(const AA: TFidoAddress; Status:
  TOutStatus);
var
  S: string;
  I: DWORD;
...
begin
  ...
  if not InputSingleAddress(LngStr(rsMMeditFReq), AA, nil) then
    Exit;
...
begin
  Result := False;
  if not InputNewPwd(Key, LngStr(rsMMMPEntrNew), False, IDH_MASTPWD) then
    Exit;
...
begin
  if WinDlgCapHlp(LngStr(rsMMMPCreate), MB_YESNO or MB_HELP or MB_ICONWARNING,
    Handle, LngStr(rsMMMPStUpCap), IDH_MASTPWD) <> mrYes then
    Exit;
  if InputMasterPassword(Handle) then
    DisplayInfoLng(rsMMMPStUpOK, Handle);
end;

procedure TMailerForm.mcMasterPwdChangeClick(Sender: TObject);
begin
  if WinDlgCap(LngStr(rsMMMPCfmChange), MB_YESNO or MB_ICONWARNING, Handle,
    LngStr(rsMMMPChgCap)) <> mrYes then
    Exit;
  if ReloginFailed then
    Exit;
  if InputMasterPassword(Handle) then
    DisplayInfoLng(rsMMMPChgOK, Handle);
end;

procedure TMailerForm.mcMasterPwdRemoveClick(Sender: TObject);
begin
  if WinDlgCap(LngStr(rsMMMPCfmRemove), MB_YESNO or MB_ICONWARNING, Handle,
    LngStr(rsMMMPRemoveCap)) <> mrYes then
    Exit;
  if ReloginFailed then
    Exit;
  Cfg.MasterKey := 0;
  Cfg.MasterKeyChk := 0;
  if StoreConfig(Handle) then
    DisplayInfoLng(rsMMMPRemovedOK, Handle);
end;
...
begin
  ...
  if ONode = nil then
    GlobalFail('%s', ['TMailerForm.OutMgrSelectedItem ONode = nil']);
  Result := ONode.Data;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        CurColl.Lock := True;
        if PrevColl <> nil then
          PrevColl.Unlock := True;
        PrevColl := CurColl;
      end;
    end;
    if CurColl = nil then
...
begin
  if FidoOut.MoveFiles(C, @C.Addr, @C.DestAddr, @C.Stat, @C.Stat, False, False,
    False) then
    Result := ''
  else
    Result := GetErrorMsg;
...
function PerformOutboundOp(C: TOutFileList; OpCode: TOutMgrOpCode; var s:
  string): Boolean;
type
...
procedure TMailerForm.PerformOutboundOperations(FileLists: TColl; DestAddr:
  PFidoAddress; OpCode: TOutMgrOpCode);
var
  i: Integer;
  C: TOutFileList;
...
begin
  ...
  begin
    ...
    begin
      Ignore := False;
      repeat
        if FidoOut.Lock(C.Addr, osBusy, False) then
          Break;
        case WinDlg(FormatLng(rsMMOutIsBusy, [Addr2Str(C.Addr)]), MB_ICONWARNING
          or MB_ABORTRETRYIGNORE, Handle) of
          idAbort: Exit;
          idIgnore:
            begin
              Ignore := True;
    ...
    if Ignore then
      Continue; // looking for a next node
    if DestAddr <> nil then
      C.DestAddr := DestAddr^;
    OK := PerformOutboundOp(C, OpCode, s);
    if ((not OK) or (C.Unlock)) and ((DestAddr = nil) or
      (CompareAddrs(DestAddr^, C.Addr) <> 0)) then
      FidoOut.Unlock(C.Addr, osBusy);
    if (not OK) then
    begin
      Ignore := True;
      if WinDlg(s, MB_ICONWARNING or MB_OKCANCEL, Handle) = idCancel then
        Exit;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if FidoOut.Paused(o.Address) then
      ompCfPause.Caption := 'Unpause'
    else
      ompCfPause.Caption := 'Pause'
  end
  else
  begin
  ...
  begin
    ...
    if (c <> nil) and (m.Tag <> 1) then
      h := FormatLng(rsMMOutDoNItems, [h, c.Count]);
    m.Caption := h;
    if f then
    begin
      f := False;
...
function TMailerForm.GetOutFileColl(FileMask: PString; NodeAdrr: PFidoAddress;
  OutStatus: POutStatus; var AInfo: TOutMgrGroupInfo): TOutFileColl;
var
  R: TOutFileColl;
  i,
...
begin
  ...
  begin
    N := OutMgrNodes[i];
    if (OutStatus <> nil) and (not (OutStatus^ in N.FStatus)) then
      Continue;
    if (NodeAdrr <> nil) and (CompareAddrs(N.Address, NodeAdrr^) <> 0) then
      Continue;
    for j := 0 to CollMax(N.Files) do
    begin
      F := N.Files[j];
      if (OutStatus <> nil) and (OutStatus^ <> F.FStatus) then
        Continue;
      {>¬от тут bug<}
      if (FileMask <> nil) and (not MatchMask(F.Name, FileMask^)) then
        Continue;
...
function GetCollCurrentFile(AForm: TMailerForm; Item: TOutItem; var Caption:
  string; var AInfo: TOutMgrGroupInfo): TOutFileColl;
var
...
begin
  ...
  else if not (Item is TOutFile) then
...
function GetCollByName(AForm: TMailerForm; Item: TOutItem; var Caption: string;
  var AInfo: TOutMgrGroupInfo): TOutFileColl;
var
  F: TOutFile absolute Item;
  Path,
...
function GetCollByExt(AForm: TMailerForm; Item: TOutItem; var Caption: string;
  var AInfo: TOutMgrGroupInfo): TOutFileColl;
var
  F: TOutFile absolute Item;
  Path,
...
begin
  ...
  if Caption = '' then
    Caption := LngStr(rsMMOutSameFXt);
end;

function GetCollByStatus(AForm: TMailerForm; Item: TOutItem; var Caption:
  string; var AInfo: TOutMgrGroupInfo): TOutFileColl;
var
  F: TOutFile absolute Item;
begin
  ...
  if (Item <> nil) and (Item is TOutFile) then
    case F.FStatus of
      osImmed,
        ...
        osHold:
        begin
          Caption := FormatLng(rsMMOutNFofN, [F.StatusString,
            Addr2Str(F.Address)]);
          Result := AForm.GetOutFileColl(nil, @F.Address, @F.FStatus, AInfo);
        end;
    end;
  if Caption = '' then
    Caption := LngStr(rsMMOutSameSt);
end;

function GetCollByNode(AForm: TMailerForm; Item: TOutItem; var Caption: string;
  var AInfo: TOutMgrGroupInfo): TOutFileColl;
begin
  if Item = nil then
  begin
...
function GetCollEntireOutbound(AForm: TMailerForm; Item: TOutItem; var Caption:
  string; var AInfo: TOutMgrGroupInfo): TOutFileColl;
begin
...
function TMailerForm.GetOutCollByTag(ATag: Integer; Item: TOutItem; var Caption:
  string; var AInfo: TOutMgrGroupInfo): TOutFileColl;
type
  TGetOutCollFunc = function(AForm: TMailerForm; Item: TOutItem; var Caption:
    string; var AInfo: TOutMgrGroupInfo): TOutFileColl;
...
procedure TMailerForm.FillOutMgrSubMenu(AMenu: TMenuItem; C: TOutFileColl; const
  AInfo: TOutMgrGroupInfo);
const
  idxReaddr = 0;
  idxFinalize = 1;
  idxImmed = 3;
...
begin
  ...
  begin
    if not (MessageBox(application.handle,
      'This operation will be applied to the entire outbound. Do you want to continue?',
      'Mailer', mb_OKCANCEL or MB_ICONQUESTION) = idok) then
      exit;
  end
  else
  begin
    case opcode of
      omoKill:
        if not (MessageBox(application.handle,
          'This operation will delete one or more files. Do you want to continue?',
          'Mailer', mb_OKCANCEL or MB_ICONQUESTION) = idok) then
          exit;
      omoUnlink:
        if not (MessageBox(application.handle,
          'This operation will unlink one or more files. Do you want to continue?',
          'Mailer', mb_OKCANCEL or MB_ICONQUESTION) = idok) then
          exit
        else
          ;
...
var
  ...
  o: TOutFile;
begin
  for i := 0 to CollCount(c) - 1 do
  begin
    o := c[i];
    if o.OutAttType = oatFileBox then
    begin
...
begin
  ...
  if (opCode = omoKill) and (oatFileBox in Info.OutAttTypesFound) then
    KillBoxes(c);
  i := CollCount(c);
  ...
  begin
    ...
    if not InputSingleAddress(FormatLng(rsMMOutOpRAdr, [i]), a, nil) then
      Exit;
    p := @a;
    repeat
      if FidoOut.Lock(p^, osBusy, False) then
        Break;
      if WinDlg(FormatLng(rsMMOutIsBusy, [Addr2Str(p^)]), MB_ICONWARNING or
        MB_RETRYCANCEL, Handle) = idCancel then
      begin
        FreeObject(f);
        Exit;
      end;
  ...
  if (t <> 6) or (YesNoConfirm(FormatLng(rsMMOutOpCmfEnt, [i]), Handle)) then
    PerformOutboundOperations(f, p, OpCode);
  if p <> nil then
    FidoOut.Unlock(p^, osBusy);
  FreeObject(f);
  RereadOutbound(false);
end;
...
var
  ...
  s: string;
begin
  if not GetAttachStatusEx(Status, DoPoll, @K) then
    Exit;
  repeat
    if FidoOut.Lock(A, osBusy, False) then
      Break;
    if WinDlg(FormatLng(rsMMOutIsBusy, [Addr2Str(A)]), MB_ICONWARNING or
      MB_RETRYCANCEL, Handle) = idCancel then
      Exit;
  until False;
...
begin
  ...
  D.Options := [ofAllowMultiSelect, ofFileMustExist, ofPathMustExist,
    ofHideReadOnly];
  SC := nil;
...
begin
  if not BrowseAtNode(Addr) then
    DisplayError(FormatLng(rsMMNodeUndNdl, [Addr2Str(Addr)]), Handle);
end;
...
begin
  ...
  begin
    ...
    if FStartMinimized and not IniFile.Stealth then
      PostMessage(Handle, WM_APPMINIMIZE, 0, 0);
    PostMsg(WM_IMPORTPWDL);
    WMCLOSE := false;
...
begin
  SetForegroundWindow(Handle);
  if OutMgrOutline.DroppedFiles = nil then
    Exit;
  i := OutMgrOutline.GetItem(OutMgrOutline.DropPoint.X,
    OutMgrOutline.DropPoint.Y);
  if i <> -1 then
    OutMgrOutline.SelectedItem := i;
  o := OutMgrSelectedItem;
  if o = nil then
  ...
  FillChar(A, SizeOf(A), #0);
  if not InputSingleAddress(FormatLng(rsMMAttachIA,
    [OutMgrOutline.DroppedFiles.Count]), A, p) then
    Exit;
...
begin
  SetForegroundWindow(Handle);
  if PollsListView.DroppedFiles = nil then
    Exit;
  o := PollsListView.GetItemAt(PollsListView.DropPoint.X,
    PollsListView.DropPoint.Y);
  if o <> nil then
  begin
    ...
    FillChar(A, SizeOf(A), #0);
    if not InputSingleAddress(FormatLng(rsMMAttachIA,
      [PollsListView.DroppedFiles.Count]), A, p) then
      Exit;
...
begin
  FillChar(AA, SizeOf(AA), #0);
  if not InputSingleAddress(LngStr(rsMMAttachTA), AA, nil) then
    Exit;
  AttachFilesQuery(AA);
end;
...
begin
  FillChar(AA, SizeOf(AA), #0);
  if not InputSingleAddress(LngStr(rsMMBrsNdlAt), AA, nil) then
    Exit;
...
begin
  o := OutMgrSelectedItem;
  if (o = nil) or (not (o is TOutFile)) then
    Exit;
  ...
  s := o.Name;
  if not xIsReg(ExtractFileExt(s)) then
    Exit;
  ShellExecute(Handle, { handle to parent window }
    nil, { pointer to string that specifies operation to perform }
    PChar(s),
    ...
    begin
      ...
      begin
        ...
        if (Application.MainForm = Self) and 
          (M.Msg = WM_SYSCOMMAND) and
          (M.WParam = SC_MINIMIZE) then
        begin
          TrayIcon.Active := not IniFile.Stealth;
          TNum := MainTabControl.TabIndex;
          TNam := MainTabControl.Tabs[MainTabControl.TabIndex];
          TAct := ActiveLine;
        end; // if (Application.MainForm = Self) and (M.Msg = WM_SYSCOMMAND) and (M.WParam = SC_MINIMIZE) then
    end; // if (Application.MainForm <> nil) and (TrayIcon <> nil) then
    inherited WndProc(M);
  except
    on E: Exception do
      ProcessTrap(E.Message + ' (Message: ' + IntToStr(M.Msg) + ')',
        'TMailerForm.WndProc');
  end;
end;
...
function TMailerForm.FormHelp(Command: Word; Data: Integer; var CallHelp:
  Boolean): Boolean;
begin
...
begin
  ...
  begin
    s := li.Caption;
    if not ParseAddress(s, a) then
      GlobalFail('TMailerForm.mpPauseClick, failed to parse "%s"', [s]);
    EnterFidoPolls;
    for i := 0 to FidoPolls.Count - 1 do
    begin
      p := FidoPolls[i];
...
begin
  ...
  begin
    ...
    if (i >= 0) and (i < BWZColl.Count) then
      BWZColl[i]
    else
      UpdateBWZListBox;
  end;
...
begin
  ...
  try
    BWZColl.Update;
    if BWZColl.Count - 1 < bwListView.ItemFocused.Index then
      exit;
...
begin
  ...
  begin
    ...
    if w is TLabel then
    begin
      if (w as TLabel).Font.Name <> 'Small Fonts' then
      begin
...
begin
  ...
  if IniFile.LoggerFontAttr[1] = '1' then
    LogBox.Font.Style := LogBox.Font.Style + [fsBold];
  if IniFile.LoggerFontAttr[2] = '1' then
    LogBox.Font.Style := LogBox.Font.Style + [fsItalic];
  if IniFile.LoggerFontAttr[3] = '1' then
    LogBox.Font.Style := LogBox.Font.Style + [fsUnderline];
  if IniFile.LoggerFontAttr[4] = '1' then
    LogBox.Font.Style := LogBox.Font.Style + [fsStrikeOut];
  LogBox.Text := '';
  LogBox.DoPaint(true);
...
begin
  ...
  begin
    ...
    mnuDelBWZ.Caption := Format(mnuDelBWZ.Caption,
      [BWListView.Items.Item[BWListView.ItemFocused.Index].Caption]);
  end
  else
  begin
...
begin
  ShellExecute(Handle,
    ...
    'http://bakh.spb.ru/Download/Taurus/',
    nil,
    nil,
    SW_SHOWNORMAL);
end;
...
begin
  ...
  begin
    ...
    Delete(ActiveLine.SD.Prot.Chat.DispStr,
      length(ActiveLine.SD.Prot.Chat.DispStr), 1);
    eType.Text := '';
    Key := #0;
  end;
end;
...
begin
  if xRow > 1 then
  begin
    ActiveLine.SD.Prot.ListBuf.Add('LST DEL ' + (Sender as TAdvGrid).Cells[1,
      xRow]);
  end;
end;

procedure TMailerForm.gLstRowMoved(Sender: TObject; FromIndex, ToIndex:
  Integer);
begin
  ActiveLine.SD.Prot.ListBuf.Add('LST MOV ' + (Sender as TAdvGrid).Cells[1,
    FromIndex] + ' ' + IntToStr(FromIndex - ToIndex));
end;

procedure TMailerForm.mnuClockClick(Sender: TObject);
begin
  mnuClock.Checked := not IniFile.ClockVisible;
  if Sender <> nil then
  begin
    IniFile.ClockVisible := not IniFile.ClockVisible;
    evListView.Visible := not IniFile.ClockVisible;
    with Clock, Clock2, JvxClock do
    begin
      Enabled := IniFile.ClockVisible;
      Visible := IniFile.ClockVisible;
    end;
    //    Clock2.Enabled := IniFile.ClockVisible;
    //    JvxClock.Enabled := IniFile.ClockVisible;
  end;
end;
...
begin
  ...
  b := False;
  for i := 0 to CollMax(C.Lines) do
  begin
    l := C.Lines[i];
    if ((ActiveLine <> PanelOwnerDaemon) and (ActiveLine.LineId = dword(l.Id)))
      or (l.Id = -1) then
    begin
...
begin
  ...
  if evListView.ItemFocused = nil then
    exit;
  ...
  try
    if i > -1 then
    begin
      for n := 0 to CollMax(Ev) do
      begin
        oe := Ev[n];
        if oe.FName = evListView.Items[i].Caption then
        begin
          ne := Pointer(oe.Copy);
          if not EditEvent(ne) then
            FreeObject(ne)
          else
          begin
            Ev[n] := ne;
...
begin
  SetForegroundWindow(Handle);
  if stListView.DroppedFiles = nil then
    Exit;
  o := stListView.GetItemAt(stListView.DropPoint.X, stListView.DropPoint.Y);
  if o <> nil then
  begin
    if ParseAddress(ExtractWord(4, o.Caption, [' ']), OA) then
    begin
      p := @OA;
      FillChar(A, SizeOf(A), #0);
      if not InputSingleAddress(FormatLng(rsMMAttachIA,
        [stListView.DroppedFiles.Count]), A, p) then
        Exit;
