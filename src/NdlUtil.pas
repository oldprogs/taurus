...
uses
   Recs, AltRecs, LngTools, RadIni, Wizard, CfgFiles, Plus, UWrd, xIP;
...
var
  ...
  begin
     Addr.Node := StrToInt(wordn(SSR, ',', 2, True));
  ...
  begin
     Addr.Net := StrToInt(wordn(SSR, ',', 2, True));
  ...
  begin
     Addr.Zone := StrToInt(wordn(SSR, ',', 2, True));
  ...
  begin
     Addr.Point := StrToInt(wordn(SSR, ',', 2, True));
     Inc(CompiledPoints);
  end;
...
begin
   ...
   if pos('000-', PHN) = 1 then begin
      ...
      if (n = 3) and ValidInetAddr(s) then Result := {'"' + }s{ + '"'};
   end;
end;
