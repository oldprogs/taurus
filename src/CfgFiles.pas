...
var
   IntStr: string;
begin
   IntStr := ReadString(Section, Ident, '0');
...
type
   TStrList = class(TStringList)
      procedure SaveToFile(const fName: string); override;
   end;

procedure TStrList.SaveToFile;
var
   f: integer;
   i: integer;
   s: string;
begin
   if FileExists(fName) then begin
      f := FileOpen(fName, fmOpenWrite or fmShareDenyWrite);
      if f = -1 then raise Exception.Create('SaveFile error');
      setEndOfFile(f);
   end else begin
      f := FileCreate(fName, fmCreate or fmShareDenyWrite);
      if f = -1 then raise Exception.Create('SaveFile error');
   end;
   for i := 0 to Count - 1 do begin
      s := Strings[i] + #13#10;
...
begin
   if not WritePrivateProfileString(PChar(Section), PChar(Ident), PChar(Value), PChar(FFileName)) then
...
begin
   ...
   try
      ...
      try
         Strings.Clear;
         if GetPrivateProfileString(nil, nil, nil, Buffer, BufSize, PChar(FFileName)) <> 0 then
         begin
            P := Buffer;
            while P^ <> #0 do begin
               Strings.Add(P);
...
begin
   ...
   try
      ...
      try
         Strings.Clear;
         if GetPrivateProfileString(PChar(Section), nil, nil, Buffer, BufSize, PChar(FFileName)) <> 0 then
         begin
            P := Buffer;
            while P^ <> #0 do begin
               Strings.Add(P);
