...
begin
  ...
  begin
    ...
    begin
      if L = nil then L := TStringColl.Create('TOdbcLogThread.LogErrorMsgs.L');
      L.Add(ErrLines[i]);
    end;
    ErrLines.DeleteAll;
  end;
...
begin
  ...
  begin
    r := OdbcLogRecs[i];
    if Local = nil then Local := TColl.Create('Local');
    Local.Add(r);
  end;
...
begin
  ...
  oSleep := CreateEvtA;
  OdbcLogRecs := TColl.Create('OdbcLogRecs');
  ErrLines := TStringColl.Create('ErrLines');
  Priority := tpLower;
end;
