...
uses Windows, Classes, xBase, xFido, RadIni, MClasses, Outbound, xDes, UWrd;
...
//    procedure SetRemoteVisible(v: boolean);
  ...
  TInetProtocol = class(TBiDirProtocol)
     ...
     function BoundStack(d: string): string;
     function BoundFound(d: string): boolean;
...
begin
   ...
   if fMime and (z = 'CONTENT-TRANSFER-ENCODING:') then begin
      ...
      if UpperCase(s) = 'X-UUE' then fCode := cUUE else begin
         fMime := False;
      end;
   end else
   if z = 'CONTENT-TYPE:' then begin
      ...
      a := ExtractParam('boundary', s, nBoun);
      if a <> nBoun then begin
        nBoun := BoundStack(a);
      end;
   end else
   if BoundFound(copy(z, 3, Length(z) - 2)) then begin
      fMime := True;
   end else
...
begin
   ...
   if fMime and (fCode = cB64) and ((z = '') or (z[1] = '-') or (z = '.')) then begin
      fMime := False;
      if pos('--' + UpperCase(nBoun), z) = 1 then begin
         fMime := True;
      end;
...
function TInetProtocol.BoundStack(d: string): string;
begin
  Result := d;
  if BList.IdxOf(d) = -1 then begin
     BList.Add(d);
  end;
end;

function TInetProtocol.BoundFound(d: string): boolean;
begin
  Result := BList.IdxOf(d) <> -1;
end;
