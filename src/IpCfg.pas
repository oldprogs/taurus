...
begin
   ...
   FreeObject(s);
   i := CRC32Int(Round(spIn.Value), i);
   i := CRC32Int(Round(spOut.Value), i);
...
begin
   ...
   Cfg.IPData.InC := Round(spIn.Value);
   Cfg.IPData.OutC := Round(spOut.Value);
   Cfg.IPData.BList := Round(spBL.Value);
   ...
   IniFile.InBandwidth := Round(spSPin.Value);
   IniFile.OutBandwidth := Round(spSPout.Value);
   IniFile.CashSize := Round(xSpinCash.Value) * 1024 * 1024;
