...
uses
  ...
  StdCtrls, ExtCtrls, Recs, ComCtrls;
...
implementation uses xBase, LngTools, AtomEdit, xFido;  
...
begin
   lb.Items.Clear;
   for i := 0 to Event.Atoms.Count - 1 do begin
      a := Event.Atoms[i];
      with lb.Items.Add do begin
         Caption := a.Name;
...
var
   ...
   m: string;
begin
   if Permanent then Result := LngStr(rsEvEdPerm) else begin
...
begin
   Result := False;
   if Trim(iName.Text) = '' then begin
      iName.SetFocus;
      DisplayErrorLng(rsEvNmBlnk, Handle);
      Exit;
   end;
   s := e.Text;
   if s = '' then AVl := 0 else begin
      D := Vl(s);
      if (D = INVALID_VALUE) or (D > DWORD(MaxInt)) then begin
         DisplayError(FormatLng(rsEvNotNum, [s]),Handle);
         Exit
      end;
      AVl := D;
   end;
   if (AVl > Mx) or (AVl<0) then begin
