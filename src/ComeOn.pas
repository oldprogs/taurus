...
uses
  ...
  OutBound, Windows, OdbcLog, CmdLn, Wizard, CfgFiles, Plus, Crypt;
...
begin
  SetLastError(0);
  if sss = '' then 
    inipath := MakeFullDir(JustPathname(paramstr(0)), JustFileNameOnly(ParamStr(0)) + '.ini')
  else 
    inipath := sss;
  IniName := inipath;
...
//  fShowExceptionCallback := ShowExceptionCallback;
  ...
  with TIniFile.Create(inipath) do
  begin
    s := ReadString('system', 'EMSI_CR', '%0D');
    UnpackRFC1945(s);
    sEMSI_CR := s;
    WinSockVersion := ReadInteger('system', 'WinSockVersion', 0);
    ThrTimesLog := (Win32Platform = VER_PLATFORM_WIN32_NT) and ReadBool('system','LogThreadTimes', False);
    Free;
  end;
  st2 := proc_i;
  IniFName := inipath;
...
var
  ...
  sa: TStringArray;
   i: byte;
begin
//   s := DecodeB64('PE1EQUVNT04tRjIwMDUxMjE1MjAwMS5BQTAxNTk0MzdNRDAwMTJAYXZhbnBvc3QubmV0LnJ1Pg==');
//   s := DecodeB64('IGQzNThiZGQ0MjYxNGMyZjJiYmY3NDRiM2Y0ZDgyZDEw');
   s := s + '';
   DoGetStartupInfo;
