...
uses
   Windows, Classes, SysUtils, xBase, xFido;
...
function GetBSOStatus(const n: string): TOutStatus;
...
begin
   ...
   if DstStat <> nil then begin
      ...
      osHoldMail   : os := osHold;
         ...
         begin
            GlobalFail('%s', ['TOutBound.MoveFiles, DstStat^ ??']); Exit;
         end;
      end;
   ...
   osHReq : ;
      ...
      begin
         ...
         if DstAddr = nil then begin
            if not HardCore then Result := True else begin
...
//*          AFNames.AtDelete(AFNames.IdxOf(SrcFName));
            ...
            if DS = nil then Exit;
            if SeekEOF(DS.Handle) = INVALID_FILE_SIZE then begin
               FreeObject(DS);
            ...
            osHoldMail :
               begin
                  ...
                  if not DS.WriteLn('^' + s) then begin
                     FreeObject(DS);
                     SetErrorMsg(DstFName);
                     Exit;
                  end;
   ...
   if DstAddr <> nil then begin
      ...
      if SeekEOF(AuxS.Handle) = INVALID_FILE_SIZE then begin
         FreeObject(AuxS);
         SetErrorMsg(DstFName);
         Exit;
      end;
   ...
   if DS = nil then begin
      FreeObject(AuxS);
//*    FixBoxes;
      if HardCore then begin
         for i := 0 to CollCount(AFNames) - 1 do begin
            DelFile('MoveFile4', AFNames[i]);
         end;
      end;
      Exit;
   end;
   L := TStringColl.Create;
   if not L.LoadFromStream(DS) then begin
      FreeObject(DS);
      FreeObject(AuxS);
      FreeObject(L);
//*    FixBoxes;
...
//*  Нет, а как ты предлагаешь? Последние закомментированные строчки раскомментировать?
   ...
   begin
      if not DelFile('MoveFile3', SrcFName) then begin
         SetErrorMsg(SrcFName);
         OK := False;
      end;
   end;
...
{          1:
            ...
            3:
               begin
                  if c = #3 then CopyRE := 0 else begin
                     LoChar := c; CopyRe := 2;
                  end;
               end;
            4:
                              ...
                              dmDZONE:
                                 begin
                                    ...
                                    if (v = INVALID_VALUE) and
                                      ((aa[5] = Inifile.MainAddr.Domain) or
                                       (IniFile.MainAddr.Domain = ''))
...
begin
   ...
   for i := 0 to Count - 1 do begin
      f := At(i);
      if SC.Search(@f.Name, j) then begin
         case f.FStatus of
         osImmedMail,
         osCrashMail,
         osDirectMail,
...
begin
   Result := False;
   if GetBSOStatus(n) = osNone then exit;
   s := UpperCase(n);
   a.Point := 0;
...
begin
   Result := osNone;
   s := LowerCase(wordn(n, '.', 2));
   if s = 'bsy' then Result := osBusy else
   if s = 'csy' then Result := osBusyEx else
   if s = 'req' then Result := osRequest else
...
//            _Add(3 * 24 * 3600);
         ...
         if (not Point) and (UpperCase(Xt) = '.PNT') then begin
...
begin
   case KillAction of
    kaBsoNothingAfter,
    kaAmaNothingAfter  : Result := '';
    kaBsoKillAfter     : Result := 'O/Kill';
    kaAmaKillAfter     : Result := 'A/Kill';
    kaBsoTruncateAfter : Result := 'O/Trunc';
    kaAmaTruncateAfter : Result := 'A/Trunc';
