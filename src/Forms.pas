...
begin
  ...
  begin
    ...
    if not (csInline in Result.ComponentState) and
      (CompareText(Name, Result.Name) = 0) then
      Exit;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FPosition := Value;
      if Kind = sbHorizontal then
        FControl.ScrollBy(OldPos - Value, 0)
      else
...
begin
  ...
  try
    ...
    if not (Self is TScrollBox) then
      Message.wParam := 1;
    inherited;
  finally
    Message.wParam := Save;
...
begin
  ...
  if Form <> nil then
  begin
    if Form.FActionLists = nil then
      Form.FActionLists := TList.Create;
...
begin
  ...
  if FormsFontAttr[1] = '1' then
    Font.Style := Font.Style + [fsBold];
  if FormsFontAttr[2] = '1' then
    Font.Style := Font.Style + [fsItalic];
  if FormsFontAttr[3] = '1' then
    Font.Style := Font.Style + [fsUnderline];
  if FormsFontAttr[4] = '1' then
      ...
      begin
        ...
        begin
          if Menu = AComponent then
            Menu := nil;
          if WindowMenu = AComponent then
            WindowMenu := nil;
          if ObjectMenuItem = AComponent then
            ObjectMenuItem := nil;
        end;
      end;
  end;
...
begin
  ...
  begin
    if Value and (Visible <> Value) then
      SetWindowToMonitor;
...
begin
  ...
  if FInCMParentBiDiModeChanged then
    Exit;
...
begin
  ...
  if (Parent <> nil) and (Parent = PrevParent) and
    (PrevDockSite <> HostDockSite) then
    RecreateWnd;
end;
...
begin
  ...
  if Ctl3D then
  begin
    if Color = NormalColor then
      Color := clBtnFace
  end
  else if Color = clBtnFace then
    Color := NormalColor;
end;
            ...
            begin
              if FPosition = poScreenCenter then
                SetBounds(Screen.Monitors[I].Left + ((Screen.Monitors[I].Width - Width) div 2),
                  ...
                  Width, Height)
              else if FPosition = poMainFormCenter then
              begin
                SetBounds(Screen.Monitors[I].Left + ((Screen.Monitors[I].Width - Width) div 2),
                  Screen.Monitors[I].Top + ((Screen.Monitors[I].Height - Height) div 2),
                  Width, Height)
        ...
        begin
          ...
          begin
            ...
            if FWindowMenu <> nil then
              hWindowMenu := FWindowMenu.Handle;
          end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if not SendFocusMessage(FocusHandle, CM_DEACTIVATE) then
            Exit;
        end;
        Screen.FFocusedForm := Self;
        if not SendFocusMessage(Handle, CM_ACTIVATE) then
          Exit;
      end;
      ...
      begin
        ...
        begin
          ...
          if not SendFocusMessage(FocusHandle, CM_EXIT) then
            Exit;
        end;
        ...
        begin
          ...
          if not SendFocusMessage(TempControl.Handle, CM_ENTER) then
            Exit;
        end;
...
begin
  ...
  begin
    ...
    if not (csDesigning in ComponentState) and (Menu <> nil) and
      (Menu.AutoMerge or (FormStyle = fsMDIChild)) then
      AMergeMenu := Menu;
...
begin
  if FMenu <> nil then
    FMenu.DispatchPopup(Message.MenuPopup);
end;
...
procedure TCustomForm.WMHelp(var Message: TWMHelp);
  ...
  begin
    Result := false;
    if (Control.HelpType = htContext) and (Control.HelpContext <> 0) then
      Result := true
...
begin
  Dec(FAlignLevel);
  if (FAlignLevel = 0) and (csAlignmentNeeded in FControlState) then
    Realign;
end;
    ...
    begin
      ...
      begin
        ...
        while (J < AlignList.Count) and not InsertBefore(Form,
          TCustomForm(AlignList[J]), AAlign) do
          Inc(J);
...
begin
  ...
  begin
    if NewStyleControls then
      SendMessage(FHandle, WM_SETICON, 1, 0);
...
begin
  if FMainForm = Control then
    FMainForm := nil;
  if FMouseControl = Control then
    FMouseControl := nil;
  if Screen.FActiveControl = Control then
  ...
  if Screen.FFocusedForm = Control then
    Screen.FFocusedForm := nil;
  if FHintControl = Control then
    FHintControl := nil;
  Screen.UpdateLastActive;
end;
  ...
  try
    ...
    with Message do
      case Msg of
        ...
        WM_ENDSESSION: if TWMEndSession(Message).EndSession then
            FTerminate := True;
        WM_COPYDATA:
...
begin
  ...
  if CallHelp then
    ...
    else if FHelpFile <> '' then
    begin
      HelpHandle := Handle;
      if FMainForm <> nil then
        HelpHandle := FMainForm.Handle;
      Result := WinHelp(HelpHandle, PChar(FHelpFile), Command, Data);
    end
    else if not FHandleCreated then
