...
//  Class pointers
 ...
 { --- booleans }
  ...
  private
    ...
    TimeDiff: DWORD; //–азница во времени между состо€нием
...
{    procedure DoRedir;}
    ...
    procedure SendStrLn(const S: string);
{$IFDEF USEANSI} overload;
...
procedure PurgeZombies(Force: boolean = False);
...
begin
  ...
  if MailerForms = nil then
    exit;
  b := false;
  if IniFile.ShowBalloon and (not (Application.MainForm as TMailerForm).TrayIcon.Minimized) then
    b := true;
  if IniFile.ShowBalloonMin and (Application.MainForm as TMailerForm).TrayIcon.Minimized then
    b := true;
  ...
  begin
    if (MailerThreads <> nil) then
      Application.Title := Format('Taurus: %d lines', [MailerThreads.Count]);
    if Application.MainForm <> nil then
    begin
      if (Application.MainForm as TMailerForm).TrayIcon <> nil then
      begin
...
function DeletePolls(const List: string; Log: TAbstractLogger): integer;
var
  i: Integer;
  p: TFidoPoll;
begin
  ...
  begin
    p := FidoPolls[i];
    if p.Typ <> ptpCron then
      continue;
    if (p.Owner = PollOwnerExtApp) or (p.Owner = nil) then
    begin
      if MatchMaskAddressListSingle(p.Node.Addr, List) then
      begin
        if (dword(p.Owner) > dword(nil)) and
          (dword(p.Owner) < $FFFFFFF0) then
        begin
          p.Owner.Enter;
          if (p.Owner.SD <> nil) and (p.Owner.SD.ActivePoll <> nil) then
          begin
        ...
        Log.Log(ltInfo, 'Cron poll to ' + Addr2Str(p.Node.Addr) + ' dropped');
        p.Done := pdnDeleted;
        FidoPolls.AtFree(i);
        Inc(Result);
      end;
...
begin
  ...
  else if P.FileSendDelayedNoc then
  begin
  ...
  else if P.FileSendDelayedFail then
  begin
...
var
  ...
  type
    ...
    begin
      dec(a.wDay, d);
      if (a.wDay = 0) or (a.wDay > 31) then
      begin
        DecM(a);
        a.wDay := DaysInAMonth(a.wYear, a.wMonth) + a.wDay;
      end;
      dec(a.wDayOfWeek, d);
      if a.wDayOfWeek > 6 then
      begin
  ...
  begin
    ...
    begin
      ...
      if m < DowXlt[a.wDayOfWeek] then
      begin
        DecD(a, DowXlt[a.wDayOfWeek] - m);
      end
      else if m > DowXlt[a.wDayOfWeek] then
      begin
      ...
      a.wDay := m + 1;
      if a.wDay > DaysInAMonth(a.wYear, a.wMonth) then
      begin
        a.wDay := DaysInAMonth(a.wYear, a.wMonth);
      end;
      a.wDayOfWeek := XltDow[DayOfTheWeek(SystemTimeToDateTime(a)) - 1];
      m := PrevMatch(a.wMonth - 1, c.p^[i].Months);
      if m > a.wMonth - 1 then
      begin
        DecY(a);
      end;
      a.wMonth := m + 1;
      if CalcTime(a) > Result then
        Result := CalcTime(a);
    end;
  end;
...
begin
  ...
  begin
    Result := True;
    if not AllowPermanent then
      GlobalFail('assert(%s)', [AllowPermanent]);
  end
  else
  begin
    Result := False;
    ...
    begin
      ...
      begin
        ...
        if CalcTime(c.Match) < CalcMatch(c) then
        begin
          Result := True;
          c.Delay := True;
          c.Match := t;
...
begin
  ...
  try
    Result := nil;
    for i := CollMax(OwnPolls) downto 0 do
    begin
      PP := OwnPolls[i];
      if P = PP.Poll then
      begin
...
begin
  ...
  begin
    ...
    if f = '' then
      f := d.Flags;
    if not DialAllowed(Restriction, f, s, P.Node.Addr, m) then
    begin
      if SC <> nil then
        LogThis('*', FormatLng(rsMMpiRA, [m]));
      p.Banned := True;
      Continue;
    end;
    ...
    begin
      ...
      begin
        if SC <> nil then
          LogThis('!', Format('%s (%s UTC / %s Local)', [LngStr(rsMMpiNWT), FSC62TimeToStr(t), FSC62TimeToStr(NodeFSC62TimeEx(d.Flags, P.Node.Addr, True))]));
        p.Banned := True;
        Continue;
      end;
    end;
...
//   p.Reset;
  p.Typ := ptpOutb;
  MailerTransit.Enter;
  for i := CollMax(MailerTransit) downto 0 do
  begin
    m := MailerTransit[i];
    for j := CollMax(m.TRSList) downto 0 do
    begin
      s := m.TRSLIst[j];
      ParseAddress(s, a);
  ...
  begin
      ...
      case p.Node.PrefixFlag of
        nfPvt:
          begin
            ...
            begin
              if SC <> nil then
                SC.Add(Format('The node has %s status in the nodelist. Use manual poll to connect anyway.', [cNodePrefixFlag[p.Node.PrefixFlag]]));
              Exit;
            end;
          end;
        nfHold,
    ...
    begin
      ...
      begin
        ...
        if p.FileSendDelayedBusy or p.FileSendDelayedNoc or p.FileSendDelayedFail then
        begin
    ...
    begin
      if TimerInstalled(p.LastTry) and not TimerExpired(p.LastTry) then
      begin
        if SC <> nil then
        begin
    ...
    if not PreparePoll(p, PP, NI, TQ, RR, SC, Mlr <> PollOwnerDaemon) then
      Exit;
    ...
    if (Mlr = PollOwnerDaemon) and (p.Typ <> ptpTest) and TimerInstalled(p.LastPoll) then
    begin
      if ElapsedTime(p.LastPoll) < 30 * 20 then
      begin
        Result := plsHLD;
        if SC = nil then
        begin
...
begin
  ...
  EnterFidoPolls;
  try
    for i := 0 to CollMax(FidoPolls) do
    begin
      p := FidoPolls[i];
...
begin
  ...
  begin
    s := ExtractWord(1, ExtractWord(j, s2, [',']), [':']);
    n := pos(s, protstr);
    if n > 0 then
    begin
      n := posex(s, s2, p);
...
begin
  ...
  try
    ...
    begin
      ...
      if (OldTyp <> NewTyp) and (OldTyp <> ptpManual) and (OldTyp <> ptpTest) then
      begin
    ...
    begin
      ...
      MailerThreads.Leave;
      if IPPolls <> nil then
        InsertOwnPoll(IPPolls.OwnPolls, P);
      AP := P;
      if IPPolls <> nil then
      begin
        SetEvt(IPPolls.oSleep);
    ...
    if (AP.Typ = ptpTest) then
    begin
      if (OldTyp <> ptpTest) then
      begin
        AP.Reset;
      end;
...
begin
  ...
  try
    ...
    for i := CollMax(FidoPolls) downto 0 do
    begin
      p := FidoPolls[i];
      p.Keep := False;
      ...
      begin
        ...
        begin
          ...
          if not (p.Typ in [ptpCron, ptpTest, ptpManual, ptpBack]) then
          begin
            for t := Low(TPollType) to High(TPollType) do
            begin
              if (t in n.FPollType) then
                p.Typ := t;
            end;
          end;
          if p.Typ = ptpUnknown then
          begin
            p.Typ := ptpOutb;
          end;
        ...
        begin
          if (p.Typ = ptpOutb) or (p.Typ = ptpImm) or (p.Typ = ptpNetm) or (p.Typ = ptpNmIm) then
            FreePoll_I_P;
        end;
      end;
    end;
  ...
  begin
    n := c[i];
    if ExistFile(n.Name) and OutDial(n.FStatus, DirAsNormal, NormAsCrash) then
    begin
...
begin
  ...
  begin
    c := DeleteBSY;
    if c > 0 then
      ALogger.LogFmt(ltInfo, '%d old BSY-files were deleted (cron-app)', [c]);
    exit;
  end;
...
begin
  ...
  if ApplicationDowned and IniFile.Stealth then
  begin
    ShowMode := swHide;
  end;
end;
...
//   if pc.Count > 0 then begin
  if (RuningProcess) then
  begin
    QueuedProcessColl.Enter;
    try
      QueuedProcessColl.Add(Astr);
      QueuedProcessColl.Objects[QueuedProcessColl.IdxOfUC(Astr)] := CronThr.ProcsLogger;
    finally
...
begin
  Zombies.Enter;
  try
    if Force or (not TimerInstalled(LastZombiesPurged)) or (TimerExpired(LastZombiesPurged)) then
    begin
      NewTimerSecs(LastZombiesPurged, 120);
      for i := CollMax(Zombies) downto 0 do
      begin
        pi := Zombies[i];
        if not GetExitCodeProcess(pi.pi.hProcess, Code) then
          Code := 0;
  ...
  case Typ of
    ...
    piZmodem,
      ...
      begin
        ...
        if (Typ = piZmodem8K) or (Typ = piZmodem8KD) then
          Include(zmo, zm8K);
        if Typ = piZmodem8KD then
          Include(zmo, zmDirect);
        IsZModem := True;
    ...
    piHydra:
      begin
        ...
        begin
          ...
          if u then
          begin
            if uppercase(z) = 'OEM' then
              OEM := true;
            if uppercase(z) = 'DOS' then
              OEM := true;
            if uppercase(z) = 'WIN' then
              OEM := false;
            if uppercase(z) = 'SHORTLONG' then
              ShortAndLong := true;
            if uppercase(z) = 'ONLYLONG' then
              ShortAndLong := false;
          end;
        end;
        prec.HydraOEM := OEM;
...
var
  ...
  begin
    ...
    if P <> nil then
    begin
      if not (P as TTAPIPort).MakeCall then
      begin
      end;
    end;
  end;
  ...
  begin
    ...
    if H = INVALID_HANDLE_VALUE then
      Exit;
    with R.D do
      if not SetCommParams(H, Bps, Data, Parity, Stop and $0F, hFlow, sFlow) then
        Exit;
...
begin
  ...
  if Result = '' then
    Result := GetPasswordAux(Addr);
  if Result = '' then
    Result := GetPasswordGrid(Addr);
end;

function AddrRestrictionMatches(C: TFidoAddrColl; const Rqd, Frb: string): Boolean;
begin
  Result := False;
  if (Rqd <> '') and (not MatchMaskAddressListMultiple(C, Rqd)) then
    Exit;
  if (Frb <> '') and (MatchMaskAddressListMultiple(C, Frb)) then
    Exit;
  Result := True;
end;
...
begin
  AOwnPolls.Enter;
  try
    for j := CollMax(AOwnPolls) downto 0 do
    begin
      p := AOwnPolls[j];
      if p.Poll = ap then
        AOwnPolls.AtFree(j);
...
begin
  ...
  else if APort is TSerialPort then
  begin
    m.DialupLine := True;
    m.TapiDevice := False;
  end;
...
begin
  if EvtCnt > 0 then
    FreeMem(EvtIds, EvtCnt * SizeOf(Integer));
  EvtCnt := 0;
  EvtIds := nil;
end;
...
begin
  ...
  if EvtIds = nil then
    exit;
  if EventsThr = nil then
    exit;
  if EventsThr.Events = nil then
    exit;
...
begin
  ...
  repeat
    ...
    begin
      l := Pointer(Cfg.Lines.GetRecById(LineId));
      if l.Id <> LineId then
        GlobalFail('%s', ['TMailerThreadEventProcessor.Refill']);
...
begin
  ...
  ProcessColl.Enter;
  for i := CollMax(ProcessColl) downto 0 do
  begin
    ProcNfo := ProcessColl[i];
    if not GetExitCodeProcess(ProcNfo.PI.hProcess, Code) then
      Code := 0;
    if Code = STILL_ACTIVE then
      Continue;
  ...
  if not RuningProcess then
  begin
    QueuedProcessColl.Enter;
    for i := 0 to QueuedProcessColl.Count - 1 do
    begin
      if QueuedProcessColl.Objects[i] = Self then
      begin
        if ProcessColl.Count = 0 then
        begin
...
begin
  ...
  ProcessColl.Enter;
  Zombies.Enter;
  for i := 0 to CollMax(ProcessColl) do
  begin
    ProcNfo := ProcessColl[i];
...
var
  i: integer;
begin
  if trycount > 255 then
    GlobalFail('Couldn''t write to %s. TryCount exceeded 255.', [s]);
...
destructor TCronThreadLogger.Destroy;
begin
  inherited Destroy;
end;
...
begin
  T.SD.FileSkip := True;
  T.D.SkipIs := True;
  if IniFile.PollAddFlags[4] = '1' then
    T.SkipNextTic := true;
...
begin
  ...
  T.D.SkipIs := True;
  if IniFile.PollAddFlags[3] = '1' then
    T.RejectNextTic := true;
...
begin
  if (not AllowedMdmCmdState(T.State)) then
    Exit;
...
begin
  ...
  Logged := False;
  if (Owner = nil) or (Owner = PollOwnerExtApp) then
    ClearTimer(LastTry);
  TryBusy := 0;
  TryNoConnect := 0;
  TrySessionAborted := 0;
...
begin
  ...
  if (Typ = ptpTest) and (TrySessionAborted > 1) then
  begin
    s := '';
...
begin
  if (Node = nil) or (Node.DialupData = nil) then
  begin
    Result := '-Unlisted-';
    exit;
  end;
  try
    if DataIdx >= DWORD(Node.DialupData.Count) then
      DataIdx := 0;
...
begin
  try
    if DataIdx >= DWORD(Coll.Count) then
      DataIdx := 0;
...
begin
  if (Node <> nil) and (Node.IPData <> nil) then
...
begin
  ...
  begin
    ...
    for i := CollMax(OutMgrThread.Nodes) downto 0 do
    begin
      n := OutMgrThread.Nodes[i];
      if CompareAddrs(n.Address, Node.Addr) = 0 then
      begin
    ...
    for i := CollMax(FidoOut.OutCache) downto 0 do
    begin
  ...
  MailerThreads.Leave;
  if IPPolls <> nil then
    FreeOwnPoll(IPPolls.OwnPolls, Self);
  Release;
  FreeObject(Node);
  inherited Destroy;
end;
...
begin
  ...
  if Faxmodem <> nil then
    GlobalFail('%s', ['Faxmodem is not freed']);
...
begin
  LogEMSIData;
  if SD.HandshakeTimeLogged then
    Exit;
  SD.HandshakeTimeLogged := True;
  B := uGetSystemTime - SD.SessionStart - TimeDiff;
  LogFmt(ltInfo, 'Handshake time - %d seconds', [B]);
  SD.SessionStart := SD.SessionStart + TimeDiff;
end;
...
begin
  ...
  begin
    if SD.PostEMSILogErrors = nil then
      SD.PostEMSILogErrors := TStringColl.Create;
    ...
    begin
      ...
      begin // binkp
        ...
        begin
          ...
          SD.Prot.ChatBell := IniFile.ChatBell;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    if CP.Handle = INVALID_HANDLE_VALUE then
    begin
      Result := True;
      exit;
    end;
...
begin
  ...
  begin
    a := SD.rmtAddrs[i];
    if CompareAddrs(a, SD.SessionKeyAddr) = 0 then
      Continue;
    New(pa);
    pa^ := a;
    ra.Insert(pa);
  ...
  FreeObject(ra);
  if not Result then
    LogFmt(ltGlobalErr, 'Unauthorized AKA %s', [Addr2Str(a)]);
end;
...
begin
  ...
  dig := DigestToStr(D);
  if (psw = '-') or (UpperCase(dig) = UpperCase(ExtractWord(2, P.CustomInfo, [' ']))) then
  begin
    P.CustomInfo := '';
    if psw <> '-' then
    begin
...
const
  ...
  begin
    ...
    begin
      ...
      for c := 0 to CollMax(SD.rmtAddrs) do
      begin
//         OurPwd := GetBinkPPwd(SD.rmtPrimaryAddr);
        ...
        begin
          for i := 0 to Length(Uniq) div 2 - 1 do
          begin
        ...
        SD.CramMD5 := True;
        if (OurPwd <> '-') and (UpperCase(OurDig) <> UpperCase(P.CustomInfo)) then
        begin
          LogEMSIData;
        ...
        else if (OurPwd <> '-') and (UpperCase(OurDig) = UpperCase(P.CustomInfo)) then
        begin
    ...
    ChkNonEmsiPwd(P);
    if (P.CustomInfo = '') and (SD.rmtPassword <> '') then
    begin
      for i := 0 to CollMax(SD.rmtAddrs) do
      begin
        ...
        begin
          try
            for j := 0 to CollMax(n.IPData) do
            begin
              d := n.IPData[j];
              if pos('CRYPT', d.Flags) > 0 then
              begin
                P.CustomInfo := 'CRYPT';
                exit;
              end;
  ...
  begin
    r := nil;
    for i := 0 to SD.OutFiles.Count - 1 do
    begin
      r := SD.OutFiles[i];
      if pos(UpperCase(n), UpperCase(r.Name)) > 0 then
        break;
      if pos(UpperCase(n), UpperCase(r.Orig)) > 0 then
        break;
      r := nil;
    end;
    if r = nil then
    ...
    if FidoOut.Lock(r.Address, osBusy, True) then
    begin
      if FidoOut.DeleteFile(r.Address, r.Name, r.FStatus) or
        FidoOut.DeleteFile(r.Address, r.Orig, r.FStatus) then
      begin
      ...
      case r.KillAction of
        ...
        kaBsoTruncateAfter,
          kaAmaTruncateAfter:
          begin
            if TruncOutFile(r.Name) then
              s := 'truncated';
          end;
      end;
      ChkErrMsg;
      if s <> '' then
        LogFmt(ltInfo, '%s %s', [r.Name, s]);
  ...
  begin
    ...
    if ParseAddress(P.CustomInfo, a) and (SD.rmtAddrs.IndexOf(@a) > -1) then
    begin
      P.CustomInfo := 'ACK';
    end
    else if ParseAddress(P.CustomInfo, a) then
    begin
      n := FindNode(a);
      if (n <> nil) and (n.IPData <> nil) then
      begin
        InsertPoll(n, [osCrash], ptpTest);
        if MailerTransit.IndexOf(Self) = -1 then
        begin
  ...
  case AStatus of
    ...
    lfSendSync:
      begin
        if TFPos <> '0' then
        begin
          LogFmt(ltInfo, 'Transmitting from offset %s', [TFPos]);
        end;
      end;
    ...
    lfBatchReceEnd:
      begin
        ...
        if ProtCore = ptBinkP then
          TBinkP(SD.Prot).freqprocessed := true;
    ...
    lfBinkPAddr:
      if ChkBinkPAdr(Trim(P.CustomInfo), P) then
        P.CustomInfo := '';
...
begin
  ...
  for i := 0 to CollMax(OutMgrThread.Nodes) do
  begin
    n := OutMgrThread.Nodes[i];
    if CompareAddrs(Addr, n.Address) = 0 then
    begin
      for j := 0 to CollMax(n.Files) do
      begin
        o := n.Files[j];
        if o.fStatus in Stat then
        begin
...
begin
  ...
  try
    ...
    begin
      n := OutMgrThread.Nodes[i];
      if CompareAddrs(f.Address, n.Address) = 0 then
      begin
        for j := 0 to CollMax(n.Files) do
        begin
          o := n.Files[j];
...
begin
  ...
  if (SD.rmtAddrs = nil) and (SD.ActivePoll <> nil) then
  begin
  ...
  if not (rmfHFR in SD.rmtMailerFlags) then
    AddFReq;
  ...
  begin
    ...
    if TransmitRequired.Count = 0 then
      FreeObject(TransmitRequired);
    if TransmitForbidden.Count = 0 then
  ...
  if DialupLine and EP.VoidFound(eiZMHEvent) then
  begin
    FreeObject(TransmitRequired);
  ...
  begin
    ...
    if IniFile.DynamicOutbound then
      if not FidoOut.Lock(SD.rmtAddrs[i], osBusy, True) then
        continue;
    for K := 1 to N do
    begin
      S := A[K];
    ...
    IgnoreNextEvent := True;
    if Inifile.DynamicOutbound then
      FidoOut.UnLock(SD.rmtAddrs[i], osBusy);
  end;
  SD.rmtAddrs.Leave;
  ...
  begin
    Ok := False;
    for i := 1 to SD.OutFiles.Count - 1 do
    begin
      ...
      begin
        if (i = 1) and (SD.Prot <> nil) and (SD.Prot.T <> nil) and
          ...
          UpperCase(JustFileName(TOutFile(SD.OutFiles[0]).Name))) then
          continue;
        tof := SD.OutFiles[i - 1];
  ...
  SD.OutFiles.Leave;
  SD.SentFiles.Enter;
  for i := 0 to CollMax(SD.SentFiles) do
  begin
    tof := SD.SentFiles[i];
  ...
  if txMl + txFl > SD.txMail + SD.txFiles then
  begin
    SD.txMail := txMl;
    SD.txFiles := txFl;
  ...
  begin
    ...
    if ProtCore in [ptBinkP, ptPOP3, ptGATE] then
      SD.Prot.ReportTraf(txMl, txFl);
  end;
  UpdateData;
end;
...
function TMailerThread.AcceptFile(P: TBaseProtocol): TTransferFileAction;
  ...
  begin
    ...
    if (SD.SessionCore = scBinkP) and (not StrDeQuote(FName)) then
      BadFName := True
    else
      P.R.D.FName := FName;
    ...
    begin
      ...
      else if (not SD.AcceptReq) or (SD.ActivePoll <> nil) then
      begin
        SD.FreqProcessed := True;
...
begin
  ...
  if MaxSizeA < High(MaxSizeA) then
    MaxSizeA := (MaxSizeA * SD.ConnectSpeed * 60) div 8;
  if MaxSizeB < High(MaxSizeB) then
    MaxSizeB := MaxSizeB * 1024;
  TotCount := 0;
  TotSize := 0;
  etr := False;
  esr := False;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          lg(' %s (%sb, %s) attached');
          s.Add(f.FName);
          if MaxCount < High(MaxCount) then
            Inc(TotCount);
          if (MaxSizeA < High(MaxSizeA)) or (MaxSizeB < High(MaxSizeB)) then
            Inc(TotSize, f.Info.Size);
        end;
      end;
    end;
  ...
  ChkErrMsg;
  if S.Count > 0 then
    LogFmt(ltInfo, 'Total %d requested file(s) attached', [S.Count]);
  if CollMax(S) > -1 then
  begin
    ScanActive := True;
    ScanCounter := 1000;
...
begin
  ...
  begin
    ...
    if ela = 0 then
      ela := 1;
    if (ela >= IniFile.CPS_MinSecs) and (ASize >= IniFile.CPS_MinBytes) then
      Result := ASize div ela;
  end;
end;
...
begin
  ...
  begin
    fa := SD.rmtAddrs[i];
    if CompareAddrs(SD.rmtPrimaryAddr, fa) = 0 then
      Continue;
    s := s + AkaStr(fa);
  end;
  ...
  if SD.rmtTRX <> 0 then
    s := s + 'TRANX ' + Hex8(SD.rmtTRX) + CRLF;

  if SD.PasswordProtected then
    s := s + Format('Password %s' + CRLF, [SD.rmtPassword]);
...
var
  ...
  begin
    ...
    begin
      ...
      else if Action = aaAcceptLater_r then
      begin
          ...
          xstInDiskFileAppend,
            ...
            begin
              ...
              begin
                ...
                FreeBWZ(SD.WzRec);
                if IniFile.UnpackMSG and (ufe = '.PKT') then
                begin
                  UnpackPKT(fn, Logger);
                end;
              end
...
begin
  ...
  begin
    P.RemFiles.Enter;
    if UpperCase(ExtractFileExt(P.R.d.FName)) = '.PKT' then
    begin
      for ii := 0 to P.RemFiles.Count - 1 do
      begin
        if IsNetMailExt(ExtractFileExt(ExtractWord(1, P.RemFiles[ii], [' ']))) then
        begin
          OK := True;
      ...
      for ii := P.RemFiles.Count - 1 downto 0 do
      begin
        if UpperCase(ExtractWord(1, P.RemFiles[ii], [' '])) = UpperCase(P.R.d.FName) then
        begin
          OK := True;
          P.RemFiles.AtFree(ii);
          break;
  ...
  if not OK then
  begin
    if P.RecFiles = nil then
      P.RecFiles := TStringColl.Create;
    P.RecFiles.Enter;
...
begin
  ...
  begin
    Result := False;
    n := SD.OutFiles[0];
    if not (n.FStatus in MailSet) and (SD.Prot.T.d.FSize - SD.Prot.T.d.FPos > 10000) then
    begin
      for i := 1 to SD.OutFiles.Count - 1 do
      begin
        n := SD.OutFiles[i];
        if n.FStatus in MailSet then
        begin
          Result := True;
          for j := i downto 1 do
          begin
    ...
    else if (n.Status in MailSet) and
...
const
  KAS: array[TKillAction] of string = ('', ' deleted', ' truncated', '/delete', '/move', '', ' deleted', ' truncated');
  KAR: array[TKillAction] of string = ('', ' - deleted', ' - truncated', ' - delete', ' - move', '', ' - deleted', ' - truncated');
...
begin
  ...
  begin
    r := nil;
    if P.SendFTPFile then
    begin
      for i := 0 to SD.OutFiles.Count - 1 do
      begin
        r := SD.OutFiles[i];
        if Pos(UpperCase(P.CustomInfo), UpperCase(r.Name)) > 0 then
        begin
          break;
        end;
        if Pos(UpperCase(P.CustomInfo), UpperCase(r.Orig)) > 0 then
        begin
          break;
        end;
      end;
    ...
    else if (UpperCase(ExtractFileExt(r.Name)) = '.CLB') then
    begin
      SD.KillSentREQ := True;
      SD.Prot.ProtocolError := ecAbortByLocal;
    end
    else if (UpperCase(ExtractFileExt(P.T.d.FName)) = '.PKT') then
    begin
      if IniFile.UnpackMSG then
      begin
      ...
      begin
            ...
            case r.KillAction of
              ...
              kaBsoKillAfter,
                ...
                begin
                  ...
                  begin
                    if SD.HReqDelete.Search(@r.Name, I) then
                      SD.HReqDelete.AtFree(I);
                  end;
        ...
        begin
          ...
          begin
            ...
            if r.KillAction in [kaBsoKillAfter, kaAmaKillAfter] then
              FidoOut.Unlock(r.Address, osBusy);
          end;
        end;
        case Action of
          aaOK:
            begin
              ...
              begin
                i := r.Nfo.Size;
                if (r.Netm <> '') and (not (SD.SessionCore in [scNNTP])) then
                begin
  ...
  begin
    ...
    DelNodeOutbound(r);
    SD.OutFiles.Enter;
    for i := CollMax(SD.OutFiles) downto 0 do
    begin
      t := SD.OutFiles[i];
      if (t.Name = r.Name) or (t.Name = r.Orig) then
      begin
        SD.OutFiles.AtFree(i);
      end;
    end;
...
begin
  if TOutFile(p1).Nfo.Time > TOutFile(p2).Nfo.Time then
    Result := 1
  else if TOutFile(p1).Nfo.Time < TOutFile(p2).Nfo.Time then
    Result := -1
  else
    Result := 0;
end;
...
//      Inc(D.TxBytes, F.Nfo.Size);
    if SD.DisabledFiles = nil then
      SD.DisabledFiles := TStringColl.Create;
...
//      SD.txFiles := 0;
    ...
    if (SD.SessionCore in [scFTP, scHTTP]) then
    begin
      BWZColl.Enter;
      for i := 0 to CollMax(BWZColl) do
      begin
        b := BWZColl[i];
        if CompareAddrs(SD.rmtPrimaryAddr, b.Addr) = 0 then
        begin
    ...
    SD.OutFiles.Leave;
    if SD.SessionCore in [scNNTP] then
    begin
      SD.OutFiles.Sort(NNTPSort);
      while (Integer(SD.TxFiles) > IniFile.CashSize) and (SD.OutFiles.Count > 0) do
      begin
        f := SD.OutFiles[0];
        if pos('.PKT', UpperCase(f.Name)) > 0 then
        begin
  ...
  try
    ...
    P.T.D.FName := '';
    FreeObject(P.T.Stream);
    if (SD.SessionCore in [scFTP, scPOP3, scGATE, scNNTP]) and P.DummyNextFile then
    begin
      ScanOut(True);
    ...
    repeat
      ...
      begin
        ...
        begin
          f := SD.OutFiles[i];
          if SD.DisabledFiles.Found(UpperCase(f.Name)) then
          begin
            SD.OutFiles.AtFree(i);
          end;
        end;
      ...
      begin
        if not SD.SendDummyPkt then
          Exit;
        SD.SendDummyPkt := False;
        if (SD.SentFiles.Count > 0) then
          Exit;
        ss := SD.OutAddrs;
        GetWrd(ss, zzz, ' ');
        if not ParseAddress(zzz, aa) then
          aa := SD.rmtPrimaryAddr;
        Clear(ph, SizeOf(ph));
        ph.OrigNode := aa.Node;
        ...
        if (SD.rmtPassword <> '') and (Length(SD.rmtPassword) <= 8) then
          Move(SD.rmtPassword[1], ph.Password, Length(SD.rmtPassword));
      ...
      begin
        ...
        begin
          ...
          if (pos(UpperCase(P.CustomInfo), UpperCase(f.Name)) > 0) or
            (pos(UpperCase(P.CustomInfo), UpperCase(f.Orig)) > 0) then
          begin
            break;
          end;
          f := nil;
      ...
      if ((f.Link <> '') or (f.Netm <> '')) and inifile.DynamicOutbound then
      begin
        zzz := ExtractFileExt(f.Name);
        if IsArcMailExt(zzz) or (zzz = '.TMP') then
        begin
          ...
          begin
            ...
            RenameFile(f.Orig, f.Name);
            ChgNodeOutbound(f);
            if f.KillAction in [kaBsoTruncateAfter, kaAmaTruncateAfter] then
            begin
              IgnoreNextEvent := True;
          ...
          else if (f.Netm <> '') and (zzz = '.TMP') then
          begin
            f.KillAction := kaFbKillAfter;
          end;
...
begin
  ...
  if SD.Prot <> nil then
    Result := Format('%s (%s)', [Result, SD.Prot.GetStateStr]);
end;
...
begin
  if SD.AtomDisconnected then
    Exit;
  if (SD.Prot = nil) or (CP = nil) then
    Exit;
  ...
  if DU_InGr[TCPIP_GrDataSz - 1] < 0 then
    DU_InGr[TCPIP_GrDataSz - 1] := 0;
  if DU_OutGr[TCPIP_GrDataSz - 1] < 0 then
    DU_OutGr[TCPIP_GrDataSz - 1] := 0;
  if (InCPS = -1) and (OutCPS = -1) then
    Exit;
  error := False;

  repeat
    VlD := EP.DwordValueD(ACPSVlMin[SD.ActivePoll <> nil], DWORD(MaxInt));
    if VlD = DWORD(MaxInt) then
      Break;
    L := 0;
    if ((InCPS >= 0) and (DWORD(InCPS) < VlD)) then
      Inc(L);
    if ((OutCPS >= 0) and (DWORD(OutCPS) < VlD)) then
      Inc(L, 2);
    if L = 0 then
      Break;
    case L of
  ...
  if not error then
    repeat
      if SD.ConnectSpeed <= 0 then
        Break;
      EfD := EP.DwordValueD(ACPSEfMin[SD.ActivePoll <> nil], DWORD(MaxInt));
      if EfD = DWORD(MaxInt) then
        Break;
      L := 0;
      if ((InCPS >= 0) and (DWORD(InCPS) * 8 * 100 < EfD * SD.ConnectSpeed)) then
        Inc(L);
      if ((OutCPS >= 0) and (DWORD(OutCPS) * 8 * 100 < EfD * SD.ConnectSpeed)) then
        Inc(L, 2);
      if L = 0 then
        Break;
      case L of
...
begin
  ...
  begin
    if RmtPwd <> '' then
      Log(ltWarning, Format('Remote proposes password "%s"', [RmtPwd]));
  ...
  if IniFile.AllowAddPassword and (RmtPwdU <> '') and (not SD.rmtPwdAddrSet) then
  begin
...
begin
  ...
  GetWrd(sTRAF, S, ' ');
  m := V(S);
  if m = INVALID_VALUE then
    Exit;
  f := V(sTRAF);
  if f = INVALID_VALUE then
    Exit;
  D.rmtForUs := m + f;
  if D.rmtForUs > 0 then
    SD.rmtTrafInfo := Format('Remote has %sb of mail and %sb of files for us', [Int2Str(m), Int2Str(f)]);
  Result := True;
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          if trycount > -1 then
            TryWriteAgain(_EMSILogFName, EMSILogFHandle);
      ...
      if not ParseEMSILine(S, EMSI, '}') then
        Exit;
      if EMSI.Count < 9 then
        Exit;
      S := EMSI.FingerPrint;
      if not Hex2EMSI(S) then
        Exit;
      if (UpperCase(S) <> 'EMSI') then
        Exit;
      S := EMSI.AddressList;
      if not Hex2EMSI(S) then
        Exit;
      ...
      if not Hex2EMSI(s) or
        not Hex2EMSI(s2) or
        not Hex2EMSI(s3) or
        not Hex2EMSI(s4) then
        Exit;

      if s = 'Bink/+' then
        Include(SD.rmtMailerFlags, rmfNoFileDelay);
          ...
          eaTRX:
            begin
              ...
              if not Hex2EMSI(S) then
                Exit;
              SD.rmtTRX := VlH(S);
              if SD.rmtTRX = INVALID_VALUE then
                Exit;
              if SD.rmtAddrs <> nil then
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      TimeDiff := uGetLocalTime;
                      Diff := uDelphiTime(TimeDiff);
                      dt := uDelphiTime(SD.rmtTRX) + (1 / 24) * IniFile.TimeShift;
                      if Abs(Diff - dt) < 1 / 48 then
                      begin
                        if Abs(Diff - dt) > 1 / (24 * 60 * 60) then
                        begin
                          DateTimeToSystemTime(dt, SystemTime);
                          SystemTime.wHour := SystemTime.wHour + IniFile.TimeShift;
                          SetLocalTime(SystemTime);
                          TimeDiff := Integer(uGetLocalTime) - TimeDiff;
                      ...
                      begin
                        Log(ltInfo, Format('Time difference %d is too great. Sync is abandoned.', [TimeDiff - SD.rmtTRX]));
                        TimeDiff := 0;
                      end;
                    end;
                  end;
              ...
              GetBias;
              if IniFile.HydraUTCDefault then
                SD.tzshift := 0
              else
                SD.tzshift := TimeZoneBias;
            end;
          eaBTH:
            begin
              if SD.EMSI_Addons = nil then
                SD.EMSI_Addons := TStringColl.Create;
      ...
      if sTRAF <> '' then
      begin
        if not Hex2EMSI(sTRAF) then
          Exit;
        if not GetTraf(sTraf, True) then
          Exit;
      end
      else if sMOH <> '' then
      begin
        if not Hex2EMSI(sMOH) then
          Exit;
        D.rmtForUs := VlH(Copy(sMOH, 2, Length(sMOH) - 2));
        if D.rmtForUs = INVALID_VALUE then
          Exit;
        if D.rmtForUs > 0 then
          SD.rmtTrafInfo := Format('Remote has %sb for us', [Int2Str(D.rmtForUs)]);
      end;
      ...
      else if ecHFR in SD.rmtEMSICompat then
        Include(SD.rmtMailerFlags, rmfHFR);

      if SD.ActivePoll <> nil then
      begin
        if elPUA in SD.rmtLinkCodes then
          Include(SD.rmtMailerFlags, rmfPUA)
        else if elPUP in SD.rmtLinkCodes then
          Include(SD.rmtMailerFlags, rmfPUP)
        else if elNPU in SD.rmtLinkCodes then
          Include(SD.rmtMailerFlags, rmfNPU);
      end
      else
      begin
        ...
        begin
          if elHXT in SD.rmtLinkCodes then
            Include(SD.rmtMailerFlags, rmfHXT);
          if elHRQ in SD.rmtLinkCodes then
            Include(SD.rmtMailerFlags, rmfHRQ);
        end;
      end;
    ...
    if ecHYD in C then
      DP := ecHYD
    else if ecJAN in C then
      DP := ecJAN
    else if ecKER in C then
      DP := ecKER
    else if ecDZA in C then
      DP := ecDZA
    else if ecZAP in C then
      DP := ecZAP
    else if ecZMO in C then
      DP := ecZMO
    else if ecYMO in C then
      DP := ecYMO
    else
      DP := ecNCP;
...
begin
  if not SD.AcceptReq then
    Include(Cap, ecNRQ);
  if SD.Station = nil then
    GetStationData;
  if SD.ActivePoll <> nil then
    SD.OutAddrs := GetOutAKAs(SD.ActivePoll.Node.Addr);
  SetLength(B, 4096);
  ...
  if (not SD.GotNak) and (SD.ActivePoll <> nil) then
    B := EMSI_INQ + EMSI_CR + B;
  if (SD.NiagaraAllowed) and (SD.ActivePoll <> nil) then
    B := EMSI_TZP + EMSI_CR + B;
  SendStr(B);
  B := Hex4(CRC) + EMSI_CR;
  iCRC := CRC;
  SendStr(B);
...
begin
  Result := False;
  if (not IniFile.Remote_Enabled) or (IniFile.Remote_Password = '') then
    exit;
...
procedure TMailerThread.DoHSh;
  ...
  var
    ...
    AreCR: Boolean;
  begin
    if TimerInstalled(SD.HShRLast) and (ElapsedTime(SD.HShRLast) < 10) then
      Exit;
  ...
  case State of
    ...
    msHSh_s1c:
      begin
        ...
        else if SD.MayEMSI then
          SendStr(EMSI_CR)
        else if SD.MayYooHoo then
          SendStr(ccYooHoo);
        State := msHSh_sw;
      end;
    msHSh_swz:
      ...
      begin
        ...
        begin
          if (pos(#$7E#$FF#$7D, SD.InB) > 0) or
            ...
            (pos(#$7D#$20#$7D, SD.InB) > 0) then
          begin
            if (CP as TTAPIPort).Online = 1 then
            begin
  ...
  begin
    ...
    try
      month := MonthToInt(PChar(m));
        ...
        ProcessTrap(E.Message, 'MonthToInt');
    end;
    if month = 0 then
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Node <> nil then
          Log(ltEMSI, 'Displaying info from the nodelist');
      end;
      Result := Node <> nil;
    end;
  end;
...
//   SD.NeedModemStatx := True;
  ...
  begin
    if SD.ActivePoll.Node = nil then
      exit;
    SD.rmtPrimaryAddrSet := True;
    SD.rmtPrimaryAddr := SD.ActivePoll.Node.Addr;
    if SD.rmtAddrs = nil then
  ...
  begin
    if (DS.rmtStationName = '') and ObtainNode then
      DS.rmtStationName := Node.Station;
    FreeObject(Node);
    if (DS.rmtSysopName = '') and ObtainNode then
      DS.rmtSysopName := Node.Sysop;
    FreeObject(Node);
    if (DS.rmtLocation = '') and ObtainNode then
      DS.rmtLocation := Node.Location;
    FreeObject(Node);
    if (DS.rmtPhone = '') and ObtainNode then
      DS.rmtPhone := Node.Phone;
    FreeObject(Node);
    if (DS.rmtFlags = '') and ObtainNode then
      DS.rmtFlags := Node.Flags;
    FreeObject(Node);
  end;
  ...
  if SD.rmtTime <> '' then
    LogFmt(ltEMSI, '      Time : %s', [SD.rmtTime]);
      ...
      begin
        ...
        begin
          ...
          try
            ...
            begin
              dt := EmsiXDateTimeToDateTime(SD.rmtTime, False);
              if dt = -1 then
              begin
              ...
              begin
                ...
                Diff := uDelphiTime(TimeDiff) - dt;
                if (Abs(Diff) < 1 / 48) then
                begin
                  if (Abs(Diff) > 1 / (24 * 60 * 60)) then
                  begin
                    DateTimeToSystemTime(dt, SystemTime);
                    SystemTime.wHour := SystemTime.wHour + IniFile.TimeShift;
                    SetSystemTime(SystemTime);
                    TimeDiff := uGetSystemTime - TimeDiff;
                    Log(ltInfo, 'System time was synchronized with ' + Addr2Str(IniFile.Synch));
                    Log(ltInfo, Format('Time difference %d', [TimeDiff]));
                    break;
                  end
                  else
                ...
                begin
                  Log(ltInfo, Format('Time difference %d is too great. Sync is abandoned.', [TimeDiff - uGetSystemTime]));
                  TimeDiff := 0;
                end;
              end;
            end;
...
begin
  ...
  SessionIsOn := True;
  if not (SD.SessionCore in [scBinkP, scFTP, scHTTP, scSMTP, scPOP3, scGATE, scNNTP]) and (SD.ActivePoll = nil) then
  begin
...
begin
  ...
  begin
    ...
    else if (z = 'NODZA') then
    begin
      eDZA;
    end
    else if (z = 'NOZAP') then
    begin
      eZAP;
    end
    else if (z = 'NOZMO') then
    begin
      eZMO;
    end
    else if (z = 'NONIAGARA') then
      SD.NiagaraAllowed := False
    else if (z = 'NOEMSI') then
      SD.MayEMSI := False
    else if (z = 'NOYOOHOO') then
      SD.MayYooHoo := False
    else if (z = 'NODUMMYZ') then
      SD.DummyZFrb := True;
  end;
end;
...
begin
  ...
  if OutPoll then
    SetSessionKey(SD.ActivePoll.Node.Addr);
  ...
  begin
    ...
    begin
      ...
      if EP.VoidFound(eiHydra[OutPoll]) then
        Exclude(SD.OurProtocols, ecHYD);
      ...
      if EP.VoidFound(eiDummyZ[OutPoll]) then
        SD.DummyZFrb := True;
      if OutPoll then
      begin
...
begin
  ...
  begin
    ...
    ProcessColl.Leave;
    if WaitEvtA(1, @WaitEvts, 15000) = WAIT_FAILED then
      GlobalFail('%s', ['TCronThread.InvokeExec WaitFailed']);
...
begin
  ...
  if SD.SessionOK then
    t := t or cstSuccessful;
  if SD.PasswordProtected then
    t := t or cstProtected;
{$IFDEF IP}
  if not SD.Dialup then
    t := t or cstIP;
{$ENDIF}
  c.Typ := t;
...
procedure TMailerThread.DoWZ;
  ...
  begin
    ...
    begin
      ...
      if (s <> '') and (MatchMaskAddressListSingle(SD.ActivePoll.Node.Addr, s)) then
        ChatEnabled := true;
      ...
      begin
        ...
        result.ChatBell := IniFile.ChatBell;
    end
    else
    begin
      an := FindNode(SD.rmtPrimaryAddr);
      if an <> nil then
        flags := NodeFlags(an, IsDialUp);
      ChatEnabled := IniFile.ChatEnabled;
      s := EP.StrValue(eiChatEnabled);
      if (s <> '') and (MatchMaskAddressListSingle(SD.rmtPrimaryAddr, s)) then
        ChatEnabled := true;
      s := EP.StrValue(eiChatDisabled);
      if (s <> '') and (MatchMaskAddressListSingle(SD.rmtPrimaryAddr, s)) then
        ChatEnabled := false;
    ...
    if not IsZModem then
      Exit;
    if SD.DummyZFrb then
      Exit;
  ...
  begin
    Result := False;
    if SD.YooHooPkt.d.signal <> $6F then
      Exit;
    ...
    if not ChkAddrStr(Addr2Str(Addr)) then
      Exit;
    if not ChkNonEmsiPwd(nil) then
      Exit;
    if SD.BadPassword then
      Exit;

    if SD.YooHooPkt.d.hello_version <> 1 then
      Exit;

    i := SD.YooHooPkt.d.capabilities;
    if (i and yhDOES_Hydra <> 0) and (ecHYD in SD.OurProtocols) then
      DP := ecHYD
    else if (i and yhZED_ZAPPER <> 0) and (ecZAP in SD.OurProtocols) then
      DP := ecZAP
    else if (i and yhZED_ZIPPER <> 0) and (ecZMO in SD.OurProtocols) then
      DP := ecZMO
    else
      DP := ecNCP;
  ...
  procedure SendYooHooPkt;
    ...
    begin
      ...
      if D > 0 then
        Move(Password[1], Pkt.My_Password, D);
      D := MinD(20, Length(SD.Station.Sysop));
      if D > 0 then
        Move(SD.Station.Sysop[1], Pkt.SysOp, D);
      if SD.ActivePoll = nil then
        OurProts := [SD.DesiredProtocol] * SD.OurProtocols
      else
        OurProts := SD.OurProtocols;
      i := yhWZ_FREQ;
      if ecHYD in OurProts then
        i := i or yhDOES_Hydra;
      if ecZAP in OurProts then
        i := i or yhZED_ZAPPER;
      if ecZMO in OurProts then
        i := i or yhZED_ZIPPER;
      Pkt.Capabilities := i;
      Result := False;
    end;
  ...
  begin
    ...
    if s = '' then
      Exit;
    if MatchMaskAddressListSingle(SD.ActivePoll.Node.Addr, s) then
      SD.Prot.CramDisabled := True;
  end;
  ...
  begin
    ...
    begin
      p := FidoPolls[i];
      if (p.Typ <> ptpTest) and (p.FileSendDelayedNoc or FidoOut.Paused(p.Node.Addr)) then
      begin
        if SD.Prot.TRSList.Matched(Addr2Str(p.Node.Addr)) = -1 then
        begin
          if not SD.rmtAddrs.Search(@p.Node.Addr, j) and not MatchMaskAddressListSingle(p.Node.Addr, SD.OutAddrs) then
          begin
...
//            SD.SessionStart := uGetSystemTime;
    ...
    msStartWZ:
      begin
        ...
        SD.Accumulate := False;
        Priority := tpNormal;
        if SD.SessionStart = 0 then
        begin
          SD.SessionStart := uGetSystemTime;
        end;
        if ProtCore = ptBinkP then
          SD.SessionCore := scBinkP
        else if ProtCore = ptFTP then
          SD.SessionCore := scFTP
        else if ProtCore = ptHTTP then
          SD.SessionCore := scHTTP
        else if ProtCore = ptSMTP then
          SD.SessionCore := scSMTP
        else if ProtCore = ptPOP3 then
          SD.SessionCore := scPOP3
        else if ProtCore = ptGATE then
          SD.SessionCore := scGATE
        else if ProtCore = ptNNTP then
        ...
        begin
          ...
          else if SD.SessionCore = scHTTP then
          begin
            SD.Prot.BinkPTimeout := 120;
          end
          else if SD.SessionCore = scSMTP then
          begin
            SD.Prot.BinkPTimeout := 120;
            if SD.ActivePoll <> nil then
            begin
          ...
          else if SD.SessionCore = scPOP3 then
          begin
            SD.Prot.BinkPTimeout := 120;
            if SD.ActivePoll <> nil then
            begin
          ...
          else if SD.SessionCore = scGATE then
          begin
            SD.Prot.BinkPTimeout := 120;
            if SD.ActivePoll <> nil then
            begin
          ...
          else if SD.SessionCore = scNNTP then
          begin
            SD.Prot.BinkPTimeout := 120;
            if SD.ActivePoll <> nil then
            begin
          ...
          if SD.ActivePoll = nil then
          begin
            if EP.VoidFound(eiAccNoCram) then
              SD.Prot.CramDisabled := True;
          end
          else
          begin
    ...
    msFinishWZ:
      begin
        ...
        if SD.SessionCore <> scEmsiWz then
          LogEMSIData;
        if (not SD.SessionOK) and (SD.Prot <> nil) then
          Log(ltInfo, SProtocolError[SD.Prot.ProtocolError]);
        if (SD.Prot <> nil) and (SD.Prot.CP <> nil) then
        begin
          Enter;
        ...
        begin
          ...
          else if ((SD.ActivePoll <> nil) and not ValidConnection) then
    ...
    msOneWayTxBatch1,
      ...
      begin
        ...
        if SD.ActivePoll = nil then
          StatusMsg := StatusMsg + ' Server'
        else
          StatusMsg := StatusMsg + ' Client';
        ...
        if SD.PasswordProtected and IniFile.RequestTRS and (SD.Prot.ID = piBinkp) and (not SD.Prot.TransitRequested) then
        begin
          FixTRS;
        end;
        if SD.Prot.ID in [piRCC, piFTP, piHTTP, piSMTP, piPOP3, piGATE, piNNTP] then
          if SD.Prot.NextStep then
          else if SD.Prot.ProtocolError = ecOK then
            State := msWZOK
          else
            State := msFinishWZ
        else if SD.Prot.NextStep then
        begin
...
//            ScanOut(False);
    ...
    msEMSI_c2:
      begin
        ...
        begin
          if SD.Tries > 1 then
            LogFmt(ltWarning, 'EMSI data send - retry %d', [SD.Tries - 1]);
          State := msEMSI_c2a;
        end;
      end;
    msEMSI_c2a:
...
begin
  ...
  begin
    ...
    begin
      ...
      if b then
      begin
        if Sess and (APrimaryAddr.Zone <> 0) and MatchMaskAddress(APrimaryAddr, z) then
        begin
          if not en.Found(cbi) then
            en.Ins(cbi);
        end;
...
begin
  ...
  begin
    ...
    begin
      r := BWZColl[i];
      if r.Locked or (r.FSize <> r.TmpSize) then
        Continue;
      ...
      begin
        if (not ChkErrMsg) and (FLog) then
          LogFmt(ltWarning, '%s is still pending', [s]);
        Inc(Pending)
      end;
    end;
    if FLog then
    begin
      ...
      if (Pending <> 0) or (i <> 0) then
        Msg := Format('[WZ] pending: %d, incomplete: %d', [Pending, i]);
    end;
...
begin
  ...
  if not Result then
    Exit;
  if SD.RcvdNames = nil then
    SD.RcvdNames := TStringColl.Create;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not Mlr.DialupLine then
        Mlr.SD.ExtAppCloseSerial := False;
      if Mlr.CP = nil then
      begin
        InheritHandles := False;
      end
...
var
  ExitCode: DWORD;
begin
  if not GetExitCodeProcess(Nfo.hProcess, ExitCode) then
    ExitCode := 0;
  if (ExtPoll <> nil) and (ExtPoll.Node.Ext <> nil) then
    FinalizeExtPoll(ExtPoll, ExitCode);
  ...
  case i of
    ...
    WAIT_TIMEOUT:
      begin
        ...
        if ExitCode = INVALID_VALUE then
          ExitCode := 0;
      end;
  end;
  if not TerminateProcess(hProcess, ExitCode) then
    Exit;
...
begin
  ...
  if pos(#8, RemCtrlBuf) <> 0 then
    b := ProcessBS(RemCtrlBuf);
    ...
    msSCCopyF_1:
      begin
        ...
        begin
          ...
          else if length(trim(RemCtrlBuf)) > 0 then
          begin
            State := msSCCopyF_2;
    ...
    msSCNewD_1:
      begin
        ...
        begin
          ...
          if (not CmdAccepted) and (length(trim(RemCtrlBuf)) > 0) then
            SendStrLn(CRLF + 'Error creating directory');
          State := msSC_0;
        end
        else
          _read_in;
    ...
    msSCRemD_1:
      begin
        ...
        begin
          ...
          if (not CmdAccepted) and (length(trim(RemCtrlBuf)) > 0) then
            SendStrLn(CRLF + 'Error deleting directory (directory''s full or doesn''t exist)');
          State := msSC_0;
        end
        else
          _read_in;
    ...
    msSCGotoD_1:
      begin
        ...
        begin
          ...
          if (not CmdAccepted) and (length(trim(RemCtrlBuf)) > 0) then
            SendStrLn(CRLF + 'Directory doesn''t exist');
          State := msSC_0;
        end
        else
          _read_in;
...
var
  ...
  begin
    ...
    if Application.MainForm <> nil then
      PostMessage(Application.MainForm.Handle, WM_REREADOUTB, 0, 0);
  end;
              ...
              if filec.Count > 0 then
                ...
                begin
                  case TOutFile(filec.At(_i)).KillAction of
                    kaBsoNothingAfter,
                      kaAmaNothingAfter: SendStr('[NA] ');
                    kaBsoKillAfter,
                      kaAmaKillAfter,
                      kaFbKillAfter: SendStr('[KA] ');
                    kaBsoTruncateAfter,
                      kaAmaTruncateAfter: SendStr('[TA] ');
...
procedure TMailerThread.DoRemoteOutbCmd(CmdId: OutboundCmd);
  ...
  begin
    ...
    try
      ...
      OutMgrNodes := nil;
      if OutMgrThread.Nodes <> nil then
        OutMgrNodes := OutMgrThread.Nodes.Copy;
    finally
      LeaveCS(OutMgrThread.NodesCS);
    end;
    if OutMgrNodes = nil then
      exit;
      //  E := False;
...
//            SD.NeedModemStatx := true;
        ...
        if SD.HstLink then
          Exclude(SD.OurProtocols, ecHYD);
        if SD.ActivePoll = nil then
          SetStatusMsg(rsMMngIn, '')
        else if SD.ActivePoll.Node <> nil then
...
begin
  ...
  if not VirtualCD then
    Exclude(i, mrpConnect);
...
begin
  ...
  begin
    if not (i in AMask) then
      Continue;
    Z := SD.ModemRec.StdResp[Integer(I)];
    if i = mrpRing then
      Z := Z + ' TAPI_OFFERING_(OWNER) TAPI_OFFERING_(MONITOR)';
    while Z <> '' do
    begin
      GetWrd(Z, W, ' ');
      ...
      begin
        ...
        if (j > 1) and (US[j - 1] <> #13) and (US[j - 1] <> #10) then
          j := 0;
        if j <> 0 then
        begin
          RespGotMatch(j, w, i);
          Result := i;
          if copy(w, 1, 8) = 'TAPI OFF' then
          begin
            Result := mrpTapi;
          end;
          if SoundsON then
          begin
            if (Result = mrpRing) or (Result = mrpTapi) then
            begin
              PlaySnd('Ring', SoundsON);
            end
            else
            begin
              if (Result = mrpConnect) then
              begin
...
begin
  ...
  if SD.ActivePoll <> nil then
    SD.ActivePoll.IncBusyTries(SD.LastResponse);
end;
...
begin
  ...
  begin
    ...
    if DialupLine then
      Inc(SD.ActivePoll.TryDialup)
    else
      Inc(SD.ActivePoll.TryIp);
...
procedure TMailerThread.DoMisc;
  ...
  var
    ...
    begin
      ...
      if PreInit <> '' then
      begin
        if not SendModemString(Preinit) then
          Exit;
      end;
      if not SendModemString(Prefix + NodePhone + Suffix) then
        Exit;
      Result := True;
    end;
  ...
  begin
    ...
    begin
      ...
      if (CP <> nil) and TapiDevice then
      begin
        if (CP as TTAPIPort).MakeCall then
        begin
    ...
    case cn of
      ...
      mrpRing:
        begin
          ClearTmr1;
          Log(ltInfo, SD.LastResponse);
          if (State = msStartIdle_TAPI) or (State = msIdle_TAPI) then
          begin
            SD.RingsDone := 1;
          end;
  ...
  begin
    if (CP <> nil) and (CP.Handle <> INVALID_HANDLE_VALUE) then
    begin
  ...
  begin
    ...
    if SD.rmtPrimaryAddrSet then
      s := s + Format(' (%s)', [Addr2Str(SD.rmtPrimaryAddr)]);
  ...
  var
    ...
    begin
      ...
      if _LogOK(LogName, LHandle) then
      begin
  ...
  begin
    ...
    try
      ...
      repeat
        ...
        else if c = '%PARSE' then
        begin
          sc := s[1];
          if not compare(sc[i], m) then
          begin
        ...
        begin
          ...
          begin
            EnterFidoPolls;
            for j := 0 to CollMax(FidoPolls) do
            begin
              p := FidoPolls[j];
              if MatchMaskAddressListSingle(p.Node.Addr, sc[i]) then
              begin
                NewTimerSecs(p.LastTry, inifile.FPFlags.StandoffFail * 60);
...
//                  s := EscapeChar + EscapeChar + EscapeChar;
          ...
          begin
            c := 0;
            while not SendModemString(s) do
            begin
              HangupModem;
              inc(c);
              if c = 3 then
    ...
    msInit:
      begin
        ...
        begin
          c := DeleteBsy;
          if c > 0 then
            LogFmt(ltInfo, '%d old BSY-files were deleted (event)', [c]);
        end;
        if EP.VoidFound(eiDelPOL) then
        begin
          c := DeletePolls(EP.StrValueD(eiDelPOL, '*'), Logger);
          if c > 0 then
            LogFmt(ltInfo, '%d cron polls were deleted (event)', [c]);
        end;
    ...
    msInitModemA:
      begin
        ...
        begin
          ...
          begin
            if not SD.InitModemLogged then
              LogModemInit;
    ...
    msStartIdle:
      begin
        ...
        if (CP <> nil) and TapiDevice then
        begin
          if (CP as TTAPIPort).NeedDrop then
          begin
    ...
    msIdle: // waiting for a call
      begin
        ...
        if not TimerInstalled(SD.TmrReInit) then
          NewTimerAvg(SD.TmrReInit, CReinitTime)
        else if not TimerExpired(SD.TmrReInit) then
          DoIdle
        else
        begin
          ...
          begin
            ...
            if (EP.VoidFound(eiAccNoIncoming)) then
              State := msInitFreeCP; // by Sergey Shumakov
          end
          else
          begin
            SD.Tries := 3;
        ...
        else if (not Inifile.RASEnabled) and (not RASInvoked) and (EP.VoidFound(eiRASEvent)) then
        begin
          RASInvoked := True;
    ...
    msStartIdle_TAPI:
      begin
        if (CP as TTAPIPort).RealOwner then
          SetTmr1(10, msIdleT_Expired);
        State := msIdle_TAPI;
    ...
    msIdle_TAPI:
      begin
        ...
        begin
          (CP as TTAPIPort).DropCall;
          if ((CP as TTAPIPort).Call = 0) and (D.TimeSaved) then
          begin
            State := msInit;
    ...
    msStartDial:
      begin
        if (CP <> nil) and TapiDevice then
        begin
          if (CP as TTAPIPort).Answer then
          begin
        ...
        if (SD.ActivePoll.Node.IPData <> nil) and (SD.ActivePoll.TryIp = 0) and (uGetSystemTime - SD.ActivePoll.Birth < 60) then
        begin
          SD.ActivePoll.Release;
          SetStatusMsg(rsMMIdle, '');
          State := msStartIdle;
        ...
        SD.ConnectSpeedGot := False;
        ClearTmr1;
        if SD.ActivePoll.Node.Ext = nil then
        begin
    ...
    msStartDialPhone:
      begin
        ...
        if (CP <> nil) and TapiDevice then
        begin
          if not (CP as TTAPIPort).MakeCall then
          begin
    ...
    msStartDialOK:
      begin
        ...
        s := '';
        if (SD.ActivePoll <> nil) and (SD.ActivePoll.Node <> nil) then
        begin
    ...
    msStartAnswer:
      begin
        if (CP <> nil) and TapiDevice then
        begin
          if (CP as TTAPIPort).Call = 0 then
            State := msInit;
          if (CP as TTAPIPort).FHandle = INVALID_HANDLE_VALUE then
            exit;
        end;
        CheckModem;
        SD.InB := '';
    ...
    msCarrierLost:
      begin
        if TapiDevice and (CP as TTAPIPort).fHandOf then
        begin
          State := msIdle_TAPI;
        end
        else
...
//            SaveTarifLog(self); // visual
    ...
    msSE_OKc:
      begin
        ...
        CloseSession;
        if IniFile.SessionOKFlag then
          CreateFlag('session.ok');
        State := msInit;
    ...
    msSE_SessionAborted:
      begin
        ...
        CloseSession;
        if IniFile.SessionAbortedFlag then
          CreateFlag('session.fail');
        DoSE_SessionAborted;
        State := msInit;
    ...
    ms_WrongOutDial:
      begin
        ...
        if DS.rmtAddressList <> '' then
          LogFmt(ltGlobalErr, 'Session with %s when dialled %s - disconnecting', [DS.rmtAddressList, Addr2Str(SD.ActivePoll.Node.Addr)]);
        State := msSE_SessionAborted;
      end;
    ms_NoValidAddr:
      begin
...
var
  us: TUpdateTermStruc;
begin
  if (pos('<', AStr) > 0) and (pos('<<<', SD.InB) > 0) then
  begin
    HideInput := True;
  end;
  if HideInput then
...
begin
  if SD = nil then
    Exit;
  if (SD.ModemRec <> nil) and TimerInstalled(SD.LastModemUpd) and (not TimerExpired(SD.LastModemUpd)) then
    Exit;
...
begin
  ...
  if TapiDevice then
    (CP as TTAPIPort).WakeEvent := oEvt;
  EvtNew := False;
  ...
  begin
    ...
    if SD.ActivePoll <> nil then
      s := s + Format(' (%s)', [Addr2Str(SD.ActivePoll.Node.Addr)]);
    LogDaemon(s);
    Log(ltInfo, 'Begin v' + ProductVersion);
  end
  else
...
begin
  ...
  if D = DWORD(MaxInt) then
    Exit;
  I := uGetSystemTime - SD.SessionStart;
  if I < 0 then
    Exit;
  if (DWORD(I) < D * 60) then
    Exit;
...
begin
  ...
  try
    ...
    if SD.txMail + SD.txFiles > PublicD.txTot then
    begin
      PublicD.txTot := SD.txMail + SD.TxFiles;
    end;
    FreeObject(PubBatchT);
    ...
    begin
      ...
      if SD.Prot.T <> nil then
        PubBatchT := SD.Prot.T.Copy;
      if SD.Prot.R <> nil then
        PubBatchR := SD.Prot.R.Copy;
    end;
  finally
    LeaveCS(DisplayDataCS);
  end;
...
type
  ...
  begin
    Result := $99999999;
    if not ((T.wMonth - 1) in Cron.Months) then
      exit;
    if not ((T.wDay - 1) in Cron.Days) then
      exit;
    if not (T.wDayOfWeek in Cron.Dows) then
      exit;
  ...
  begin
    ...
    Events.Enter;
    try
      for k := 0 to CollMax(Events) do
      begin
        ol := Events[k];
        if ol.CronRec.IsPermanent then
          continue;
        if ol.Active then
          continue;
        if ol.CronRec.IsUTC then
          GetSystemTime(T)
        else
          GetLocalTime(T);
        for j := 0 to ol.CronRec.Count - 1 do
        begin
  ...
  begin
    ...
    try
      ...
      begin
        pp := Events[k];
        if pp.CronRec = nil then
          continue;
        if pp.CronRec.IsUTC then
          GetSystemTime(T)
        else
          GetLocalTime(T);
        for j := 0 to pp.CronRec.Count - 1 do
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      else if TimerInstalled(SD.Tmr1) and (TimerExpired(SD.Tmr1)) then
      begin
        State := SD.StateTmr1;
        ClearTmr1;
      end
      else if (SD.StateDeltaDCD <> msNone) and (CP <> nil) and (CP.DCD <> CP.Carrier) then
      begin
        CP.Carrier := not CP.Carrier;
        State := SD.StateDeltaDCD;
        SD.StateDeltaDCD := msNone;
      end
      else if (CP = nil) or (not CP.CharReady) then
      begin
        ToSleep := MultiTimeout([D.TmrPublic, SD.Tmr1]);
        ToSleep := MinD(ToSleep, CheckEvents);
        if State = msIdle then
          ToSleep := MinD(ToSleep, MultiTimeout([SD.TmrReInit]));
        if SD.Prot <> nil then
          ToSleep := MinD(ToSleep, SD.Prot.TimeoutValue);
        if ToSleep > 0 then
        begin
          NumEvents := 1;
          ResetEvt(oEvt);
          ...
          if SD.NiagaraSession then
            ToSleep := 20
          else if SD.SessionCore = scFTP then
            ToSleep := 20
          else if SD.SessionCore = scHTTP then
            ToSleep := 20
          else if SD.SessionCore = scSMTP then
          ...
          if (ToSleep) < (MaxInt div 2) then
            Inc(ToSleep, MinD(2000, ToSleep div 8));
          ToSleep := MaxD(25, ToSleep);
          if ToSleep > 60000 then
            ToSleep := 30000;
          ...
          if Actually = WAIT_FAILED then
            GlobalFail('%s', ['TMailerThread.LetsSleep WaitFailed']);
          if Actually < WAIT_ABANDONED_0 then
          begin
            if Actually = 0 then
            begin
              if (CP <> nil) and (CP.ComErrorColl.Count <> 0) then
                LogComError;
              DoEvt;
            end
            else if Actually + DWORD(ProcessColl.Count) >= NumEvents then
            begin
              Logger.TestRunningProcesses;
              FlushLog;
            end;
...
begin
  ...
  if P = nil then
    Exit;
  if SD <> nil then
    SD.LastCallerID := TPort(P).CallerId;
...
begin
  ...
  begin
    s := SD.HReqDelete[i];
    if ALog then
      LogFmt(ltInfo, 'Deleting ERP "%s"', [s]);
...
begin
  if SD.FaxModem = nil then
    Exit;
  if IniFile.ForceAddFaxPage then
    AddFaxPage;
  WriteTIFF;
...
//         RecreatePolls(c);
    ...
    begin
      if (p <> nil) and DialupLine and EP.VoidFound(eiZMHEvent) then
      begin
        if not (osImmedMail in p.Flav) and
          not (osCrashMail in p.Flav) and
          not (osDirectMail in P.Flav) then
...
begin
  ...
  begin
    ...
    if Result <> SD.Station.Address then
    begin
      if SD.UsedAKA <> Result then
      begin
...
begin
  if Logger = nil then
    exit;
  if SD.LoggedStrs = nil then
    SD.LoggedStrs := TStringColl.Create;
...
procedure TCronThread.DoRecalc;
  ...
  begin
    ...
    begin
      ...
      if pp.CronRec = nil then
        GlobalFail('TCronThread.DoRecalc, ParseCronRec=nil; Error=%s', [Err]);
    end;
  end;
  ...
  begin
    ...
    if ProcsCron = nil then
      FreeObject(ProcsStrs)
    else if ProcsCron.Count <> ProcsStrs.Count then
      GlobalFail('TCronThread.DoRecalc RecalcProcs ProcsCron.Count(%d) <> ProcsStrs.Count(%d)', [ProcsCron.Count, ProcsStrs.Count]);
  end;
...
begin
  ...
  begin
    ...
    begin
      n := c[i];
      o := n.Files;
      for j := 0 to CollMax(o) do
      begin
        f := o[j];
        if pos('.MSG', UpperCase(f.Name)) = Length(f.Name) - 3 then
          continue;
        a := t - f.Nfo.Time;
        if a > cardinal(IniFile.DelOldAge) * day then
        begin
          if f.Netm = '' then
          begin
            FidoOut.DeleteFile(f.Address, f.Name, f.FStatus);
  ...
  if WaitEvtA(ProcessColl.Count + 1, @WaitEvts, i) = WAIT_FAILED then
    GlobalFail('%s', ['TCronThread.InvokeExec WaitFailed']);
  if Terminated then
    Exit;
...
begin
  ...
  begin
    ...
    if oc.CronRec = nil then
      GlobalFail('TEventsThread.DoRecalc, ParseCronRec=nil; Error=%s', [Err]);
    oc.Id := e.Id;
    oc.Len := e.Len;
    oc.Atoms := e.Atoms;
    oc.EName := e.Name;
...
{         kwITX,
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if MatchMaskAddressListSingle(fAddr, ExtractWord(5, s, ['|'])) then
                begin
                  s := ExtractWord(1, s, ['|']);
  ...
  else if ValidSymAddr(Addr) then
  begin
    Sym2Port(Addr, CurPort);
  end
  else if CurProt <> ptSMTP then
  begin
    Exit;
  end;
  Port := CurPort;
...
begin
  ...
  begin
    ...
    if (p <> nil) and (p.Node.Ext <> nil) then
      FinalizeExtPoll(p, 0);
  end;
  Terminated := True;
end;
...
begin
  ...
  begin
    p.Owner := PollOwnerDaemon;
    if (P.Typ <> ptpTest) or (Prot = ptBinkP) then
    begin
...
//         RecreatePolls(c);
    ...
    repeat
      ...
      begin
        if (p <> nil) and (p.Typ = ptpTest) and (p.Banned) then
        begin
          s := '';
          SendTRSMSG(p, 'NAK', s);
        end;
  ...
  b := True;
  EnterfidoPolls;
  for pc := 0 to CollMax(FidoPolls) do
  begin
    p := FidoPolls[pc];
    if (p.Typ = ptpTest) then
    begin
...
begin
  ...
  if (not ParsePortsList(@D.Ifcico, Cfg.IPData.InPorts.Ifcico)) or
    ...
    (not ParsePortsList(@D.NNTP, Cfg.IpData.InPorts.NNTP)) then
    GlobalFail('%s', ['_RunDaemon ParsePortsList ??']);
...
//      s := uFormatDateTime('dd"/"mmm"/"yyyy:hh:nn:ss ', uGetSystemTime);
    s := uFormatDateTime('dd', uGetSystemTime) + '/' +
      MonthE(MonthOf(uDelphiTime(uGetSystemTime))) + '/' +
      uFormatDateTime('yyyy:hh:nn:ss ', uGetSystemTime);
    GetBias;
    b := TimeZoneBias;
    if b < 0 then
    begin
...
begin
  ...
  if (MailerThreads.Count > 0) or
    ...
    (PortsColl.Count > 0) then
    GlobalFail('DoneMailers, MailerThreads.Count=%d, MailerForms.Count=%d, FidoPolls.Count=%d, PortsColl.Count=%d', [MailerThreads.Count, MailerForms.Count, FidoPolls.Count, PortsColl.Count]);
...
begin
  ...
  Application.ProcessMessages;
  MailerThreads.Enter;
  for i := CollMax(MailerThreads) downto 0 do
  begin
    if not TMailerThread(MailerThreads[i]).DialupLine then
...
begin
  ...
  if EndPos > StartPos then
    g.FStream.Write(P^[StartPos], EndPos - StartPos);
...
var
  pd, pr: TFaxPage;
begin
  if SD.Faxmodem.FStream = nil then
    Exit;
  ...
  begin
    ...
    pr := DoConvert;
    if pd.BadLines > pr.BadLines then
      XChg(Integer(pd), Integer(pr));
    FreeObject(pr);
    if SD.Faxmodem.Pages = nil then
      SD.Faxmodem.Pages := TColl.Create;
...
begin
  ...
  if t.bf <> INVALID_VALUE then
    Log(ltEMSI_1, 'Binary transfer : ' + sbf);
...
procedure TMailerThread.DoFax;
  ...
  begin
    repeat
      ...
      J := Pos(#10, S);
      if I = J then
        Exit; // neither CR nor LF found - return '', leave S unmodified
      if I < J then
        XChg(I, J);
      if J > 0 then
        XChg(I, J);
  ...
  begin
    ...
    if j = INVALID_VALUE then
      GlobalFail('%s', ['Unexpected Fax Hangup']);
    ...
    msFaxRecePage:
      begin
        ...
        begin
          ...
          if SD.Faxmodem.Hangup <> INVALID_VALUE then
            State := msFaxHangup;
          if SD.Faxmodem.Error then
            State := msFaxError;
...
{               CP.Carrier := CP.DCD;
        ...
        if SD.Faxmodem.InBufPos mod $100 = 0 then
          SetTmr1(20, msFaxTimeout);
        ...
        if SD.Faxmodem.InBufPos = cFaxInBufSize then
          SaveFaxBuf;
      end;
    msFaxEndPage:
      begin
...
begin
  ...
  begin
    if REP.MatchAt(NewOutput, StartPos) <> 2 then
      Break;
...
begin
  ...
  if SD.HReqDelete = nil then
    SD.HReqDelete := TStringColl.Create;
