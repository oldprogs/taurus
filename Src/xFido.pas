...
begin
   ...
   if s <> nil then begin
      try
         ...
         Result.Region := a.reg;
         Result.Hub := a.hub;
         Result.Speed := StrToIntDef(wordn(n, ',', 7), 0);
         Result.Station := StringReplace(wordn(n, ',', 3), '_', ' ', [rfReplaceAll]);
         Result.Sysop := StringReplace(wordn(n, ',', 5), '_', ' ', [rfReplaceAll]);
         Result.Phone := wordn(n, ',', 6);
         Result.Location := wordn(n, ',', 4);
         ...
         if n = 'Down' then Result.PrefixFlag := nfDown else Result.PrefixFlag := nfUnrec;
      finally
         s.Free;
      end;
   end else begin
