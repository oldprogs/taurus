...
type
  TFileBoxesForm = class(TForm)
    ...
    procedure bNodeClick(Sender: TObject);
...
implementation uses Recs, xBase, xFido, LngTools, Outbound, AdrIBox, PathNmEx;
...
begin
  ...
  fbc := TFileBoxCfgColl.Create('fbc');
...
procedure TFileBoxesForm.bNodeClick(Sender: TObject);
begin
  if InputSingleAddress(LngStr(rsMMBrsNdlAt), FidoAddr, nil) then
  begin
    gNodes.Cells[1,gNodes.row]:=Addr2Str(FidoAddr);
  end;
end;

procedure TFileBoxesForm.bDirClick(Sender: TObject);
var
  s: string;
begin
  if (PageControl.TabIndex = 0) then
  begin
    s:=Browse(Handle,gNodes.Cells[3,gNodes.row]);
    if s<>'' then gNodes.Cells[3,gNodes.row]:=s;
  end else
  begin
    s:=Browse(Handle,eRoot.Text);
    if s<>'' then eRoot.Text:=s;
  end;
end;

procedure TFileBoxesForm.PageControlChange(Sender: TObject);
begin
  bNode.Enabled:=((Sender As TPageControl).TabIndex = 0);
//  bDir.Enabled:=bNode.Enabled;
end;
