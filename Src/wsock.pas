...
function  gethostname(name: PChar; len: Integer): Integer;
begin
  if not Assigned(F_gethostname) then F_gethostname := __Load('gethostname');
// If no error occurs, socket returns a descriptor referencing the new socket.
// Otherwise, a value of INVALID_SOCKET is returned, and a specific error code can be retrieved by calling WSAGetLastError.
  if Assigned(F_gethostname) then Result := F_gethostname(name, len) else Result := integer(INVALID_SOCKET);
end;

function gethostbyname(name: PChar): PHostEnt;
begin
  if not Assigned(F_gethostbyname) then F_gethostbyname := __Load('gethostbyname');
// If no error occurs, socket returns a descriptor referencing the new socket.
// Otherwise, a value of INVALID_SOCKET is returned, and a specific error code can be retrieved by calling WSAGetLastError.
